...
Phase 8.1 adds no language keyword or public package API. The application-local `RpgDungeonGallery.Workflow` module composes existing public calls behind explicit local result codes. Its source is included directly by the gallery and both focused state-test projects. This preserves the ordinary SMILE module model while ensuring production and tests exercise one implementation.

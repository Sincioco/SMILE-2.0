...
The [RPG Management Gallery](../../examples/RpgManagementGallery/README.md) composes Smile.RPG with Smile.UI 2.0's reusable `Menu` and `MenuNavigator` Class facades across DirectX, GDI, and Web output. Battle systems, enemies, quests, application character classes, status effects, skill trees, migrations, cloud saves, mouse, and touch remain deferred.

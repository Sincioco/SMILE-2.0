...
The D-pad maps to `KEY_UP`, `KEY_DOWN`, `KEY_LEFT`, and `KEY_RIGHT`. A maps to `KEY_ENTER`, B and X map to the non-destructive `KEY_SPACE` action, and Y maps to `KEY_TAB`. The compact default does not emit `KEY_ESCAPE` because existing games may use Escape as immediate program termination. Physical Escape and number keys retain their existing keyboard behavior. Touch, pen, and primary mouse presses on a visible controller enter the same bounded, source-aware broker as keyboard input, so each press creates one `Get Key` event and `Key_Held(...)` remains true until every owning source releases. Multi-touch direction-plus-action input is supported.

The controller uses eight real accessible buttons with restrained translucent outlines, safe-area-aware portrait and landscape layouts, and at least 56-by-56 CSS-pixel targets. Landscape placement uses the dynamic visible viewport. In portrait, the game canvas and controls are vertically centered together so mobile browser chrome does not push the lower controls off screen. All D-pad arrows use one plain triangle glyph rotated with CSS, avoiding emoji-style left/right rendering. Gesture suppression is limited to the controls; browser zoom is not disabled globally. The published Web output remains exactly `index.html`, `smile-runtime.js`, `game.js`, and `smile.css`, and the same `.smile` source continues to target Windows and Web.

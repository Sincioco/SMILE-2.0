...
`Articulation` is the reusable rigid-rig seam for deliberately faceted characters and creatures. It solves a segment center, child-joint endpoint, and renderer rotation from a parent joint, length, forward swing, and side swing. Deterministic cycle, positive-cycle, and bounded action-envelope helpers let applications animate segmented arms, legs, tails, necks, and wings at fixed simulation steps. It owns no objects and allocates no per-frame state, so games remain free to choose their meshes, materials, joint topology, and animation vocabulary.

...
`src\Smile.Language` owns source documents, tokenization, keyword and built-in facts, parsing, syntax nodes, diagnostics, modules/imports/visibility, symbols, types, package validation, project graphs, and semantic analysis. Each physical source becomes its own syntax tree. A shared module processor resolves providers and qualified public access into stable semantic identities, then one compilation-wide bound model feeds both emitters. `smilec` and the Visual Studio extension both call `SmileLanguage.Analyze`; there is no editor-only parser or duplicate semantic implementation.

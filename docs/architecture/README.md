...
The Visual Studio extension embeds the same compiler/runtime payload and registers a focused `.smileproj` project factory. One shared source-set parser and project-file editor apply `<StartupFile>` and `StartupOnly="true"` consistently, preserve unrelated properties/assets, normalize project-relative source paths, save all open participating files, and pass repeated `--source` arguments for both Windows and Web. Project, source-file, and folder context menus route build, add, startup/support, non-destructive remove, and folder commands through the hierarchy; every mutation reloads that hierarchy and the shared workspace before the next command or launch.

The editor workspace retains current text snapshots for every open project buffer. A buffer change invalidates analysis caches for the other participating files after the normal debounce. The selected startup uses ordinary supports; an unselected `StartupOnly` file is instead analyzed as a hypothetical startup with those same supports and without the selected complete program. Missing project sources produce a physical-file `SML0001` diagnostic rather than falling back to unrelated single-file semantics. Loose-file analysis remains unchanged.

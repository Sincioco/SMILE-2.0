...
The global live limits are 128 meshes, 1,024 objects, 64 models, 128 textures, 512 native materials (128 on the held Web backend), 64 legacy skeletons, 128 legacy clips, and 128 total animators. SM3D v2 remains limited to 16 parts, 64 materials, and 128 metadata texture references; only the deduplicated owned texture count consumes the global texture pool. An animated model additionally owns at most 256 nodes, 128 bones, 64 clips, 64 events per clip, and 64 sockets inside the unchanged 16 MiB file ceiling.
...
Native material handles use nine slot bits and sixteen generation bits, supporting all 512 slots without aliasing. Other resource handle layouts are unchanged. `MaximumMaterialCount3D` exposes the active backend capacity. The native pool regression fills all slots, draws with the highest slot, rejects a stale recycled handle, and returns to zero live materials. Web adoption remains held.

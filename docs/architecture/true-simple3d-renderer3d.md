...
The public diagnostics expose live counts, fixed capacities, handle validity, reference counts, PBR/simple draw counts, triangles, lights, samplers, imported ownership, animation palettes, logical resource epoch/frame state, logical/physical/provisional M5 submissions, in-flight references, snapshot bytes, effective targets, target bytes, and independent fallbacks. A live object retains its mesh, material, and optional animator; a material retains textures; and an SM3D v2 model atomically owns its part meshes, imported materials/textures, and immutable animation payload. An accepted draw separately captures its complete render state and increments in-flight counts on the mesh and each distinct texture. The source object, material, and animator may then change or be destroyed without changing the accepted draw, while mesh/texture/model destruction or mesh mutation that would invalidate borrowed storage is refused until rollback or frame end. `ResetRenderer3D` ends any active 3D pass, rolls back an open group, clears fixed snapshot/palette storage, releases shadow/HDR/bloom targets, destroys objects before animators and models, then releases materials, textures, meshes, legacy clips, and skeletons; it invalidates every handle, advances the resource epoch once, and leaves Renderer2D available. Device/context loss does not advance the epoch because retained logical resources remain valid and recreate GPU state lazily. Native object handles reserve ten slot bits so every entry in the 1,024-object pool remains reachable while retaining generation-based stale-handle rejection; smaller resource pools keep the original eight-bit layout. Web handles are never reused and deleted handles remain invalid.
...
The compiler bridge is append-only. Numeric commands now occupy 1-118, image commands 1-2, and text commands 1-9. M5 adds `113 CONFIGURE_POST`, `114 CONFIGURE_SHADOW`, `115 SET_SHADOW_AREA`, `116 SET_OBJECT_SHADOWS`, and `117 M5_VALUE`; M5.1 adds only `118 SUBMISSION_GROUP`, multiplexing begin/commit/rollback through operation `1/2/3` and a capacity or frame-serial token. Native dispatch is the `smile_graphics3d_command` switch in `graphics3d_directx.cpp` through the declarations in `graphics3d.h`. Web dispatch is the generated `renderer3DCommand` switch in `WebOutputWriter.cs`; both are reached through the existing game-window numeric bridge emitted by the compiler. The next free IDs are numeric 119, image 3, and text 10. The exact positional mapping, properties, errors, and fallback bits are recorded in the M5 and M5.1 implementation reports.

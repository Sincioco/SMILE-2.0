...
The compiler bridge is append-only. Numeric commands currently occupy 1-112, image commands 1-2, and text commands 1-9. M3 adds numeric 98-109 for model animation metadata, model animators, playback/crossfade state, FIFO/root/socket operations, capability, and palette diagnostics. M3.1 adds numeric 110 for production animator diagnostics and 111 for atomic event-state clearing. M4.1 adds numeric 112 as a read-only renderer-state query: property 1 returns the logical resource epoch and property 2 returns frame-active state. Neither milestone adds an image command. Text commands 4-9 provide exact clip/socket/event names plus named play, crossfade, and event take. M4 adds no command because `Scene3D` and `Character3D` compose the public `Graphics3D` API in ordinary SMILE source. The next free IDs are numeric 113, image 3, and text 10. The positional mapping and native/Web dispatch paths are recorded in the implementation reports.

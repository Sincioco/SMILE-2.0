...
`libraries\Smile.Text.Extras` and `examples\Phase3ABasics` prove formatVersion 2 typed public APIs through both project and package references on Windows and Web. Imported completion presents signatures such as `SUB Replace(BYREF Value AS TEXT, NewValue AS TEXT)` and `FUNCTION JoinFive(...) AS TEXT` while retaining public-only and direct-provider filtering.

Future phases may add user-defined `TYPE` records, `Smile.UI.Window`, `Smile.UI.Menu`, `Smile.RPG.Inventory`, and `Smile.RPG.Abilities`. They are not implemented in Phase 3A. Future RPG APIs and documentation use Magic Points (MP).

...
The maximum Phase 7 format-2 payload is 32,436 bytes, below the package's 36,864-byte codec buffer and the 1 MiB Phase 4 Data limit. The decoder rejects bad magic/version/schema, truncation, trailing bytes, negative or excessive counts, duplicate keys, nonpersistent actor records, unknown or mismatched definitions, incoherent equipment, invalid ranges, and capacity overflow.

The 32,436-byte value is exercised by an executable maximum-state test containing the full Phase 6 definition/progress capacities plus 16 scenes, 64 spawns/transitions/persistent actors, 128 flags, 64 values, and 16 encounter zones. Decode resets progress, restores Character values and Party/Gold, restores Equipment through the normal transactional Equip operation while the bag is empty, then restores saved Inventory, learned Abilities, Shop stock, World, Story, and Encounter progress. This order avoids a temporary 65th Inventory entry or extra copy in a full stack. Any apply failure restores the exact pre-load snapshot across all modules.

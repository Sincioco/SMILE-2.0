# Smile.RPG 1.3.0 battle API

Phase 9 introduced these four ordinary, source-authored modules in 1.2.0. The
1.3.0 format-6 package preserves that public API and adds optional,
progression-aware basic-Attack accuracy. They are
...
- actions: `QueueAction`, `StartRound`, `Advance`, `ClearQueuedActions`,
  `ConfigureAttackBaseHitChance`, `AttackBaseHitChance`,
  `ConfigureAttackProficiencyBonus`, `AttackProficiencyBonus`,
  `CalculatedAttackHitChance`
...
Basic Attack accuracy is opt-in per battle side. The default baseline is 100%,
which preserves legacy guaranteed hits and consumes no accuracy roll. Below 100,
a party actor's effective chance is
`clamp(5, 100, baseline + Level / 3 + proficiency + (AgilityA - AgilityT) / 6 + (LuckA - LuckT) / 8 + (StrengthA - DefenseT) / 20)`.
Enemy actors use the same statistic adjustments without the party-only Level
term. Participant statistics already include equipped Strength, Defense,
Agility, and Luck bonuses frozen at battle start, so character growth and gear
directly reduce miss frequency. `ConfigureAttackProficiencyBonus` is the explicit
per-participant seam for application-owned weapon mastery, class skills, buffs,
or penalties. A deterministic roll from 0 through 99 hits when it is below the
effective chance. A miss emits `BATTLE_EVENT_MISS`, applies no damage, and maps
only to a renderer-neutral message cue; applications therefore do not need to
infer whether contact VFX should play.
...
The caller supplies a nonnegative battle seed. The internal sequence is `seed = (seed * 25173 + 13849) mod 65536`. It drives damage variance, Run rolls, and opted-in basic-Attack accuracy rolls, so the same definitions, state, commands, and seed produce the same events and outcome on native and Web.

...
try:
 scene=bpy.context.scene;low=bpy.data.objects['Mira2.Repair.Low'];high=bpy.data.objects['Mira2.Repair.High']
 ...
 for channel,bake_type in [('BaseColor','EMIT'),('Normal','NORMAL'),('Roughness','ROUGHNESS')]:
  ...
  image.filepath_raw=str(root/('mira2-repaired-'+channel.lower()+'.png'));image.file_format='PNG';image.save();image.pack()
 ...
 bpy.ops.export_scene.gltf(filepath=str(root/'mira2-repaired-unrigged-body.glb'),export_format='GLB',use_selection=True,export_animations=False)
 bpy.ops.export_scene.fbx(filepath=str(root/'Mira2-Repaired.fbx'),use_selection=True,object_types={'MESH'},add_leaf_bones=False,bake_anim=False,path_mode='COPY',embed_textures=True,axis_forward='-Z',axis_up='Y')
 bpy.ops.wm.save_as_mainfile(filepath=str(root/'mira2-repaired-baked-body.blend'))
 (root/'repair-bake-status.json').write_text(json.dumps({'status':'complete','bodyTriangles':sum(len(p.vertices)-2 for p in low.data.polygons)}))
except Exception:
 (root/'repair-bake-status.json').write_text(json.dumps({'status':'failed','traceback':traceback.format_exc()}));raise

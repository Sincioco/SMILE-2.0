...
rig=bpy.data.objects['Mira2.Rig'];old=bpy.data.objects['Mira2.Staff'];scene=bpy.context.scene;scene.frame_set(1);bpy.context.view_layer.update()
hand=rig.pose.bones['mixamorig:RightHand'];grip=rig.matrix_world @ hand.matrix @ Vector((0,.075,.035));vertices=[];faces=[];colors=[]
...
tube([p+Vector((0,-.012,0)) for p in centers],[r*.33 for r in radii],4,3)
...
inverse=(rig.matrix_world @ hand.matrix @ hand.bone.matrix_local.inverted()).inverted();mesh=bpy.data.meshes.new('Mira2.ClosedStaff');mesh.from_pydata([inverse@p for p in vertices],[],faces);mesh.materials.append(old.data.materials[0]);uv=mesh.uv_layers.new(name='Palette')
...
staff=bpy.data.objects.new('Mira2.NewStaff',mesh);scene.collection.objects.link(staff);staff.parent=rig;group=staff.vertex_groups.new(name=hand.name);group.add(list(range(len(vertices))),1,'REPLACE');modifier=staff.modifiers.new('Mira2 Own Rig','ARMATURE');modifier.object=rig
bpy.data.objects.remove(old,do_unlink=True);staff.name='Mira2.Staff';bpy.context.view_layer.update()
result={'staffTriangles':len(faces),'boundaryEdges':boundary,'nonManifoldEdges':nonmanifold,'equippedTriangles':18100+len(faces)}
Path(r'D:\AI\Mira3D\Mira2\mira2-closed-staff-report.json').write_text(json.dumps(result,indent=2))

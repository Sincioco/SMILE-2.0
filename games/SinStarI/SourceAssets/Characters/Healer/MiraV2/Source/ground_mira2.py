"""Ground Mira2 from her measured body, retaining Run's intentional airborne frames."""
import bpy,json
from mathutils import Vector
from pathlib import Path
rig=bpy.data.objects['Mira2.Rig'];body=bpy.data.objects['Mira2.SkinnedBody'];scene=bpy.context.scene
if rig.get('Mira2Grounded'): raise RuntimeError('Grounding already applied; use the ungrounded checkpoint to regenerate')
def minimum():
...
rig['Mira2Grounded']=True;rig.animation_data.action=bpy.data.actions['Idle'];rig.animation_data.action_slot=bpy.data.actions['Idle'].slots[0];scene.frame_set(1)
...
Path(r'D:\AI\Mira3D\Mira2\mira2-grounding-repair.json').write_text(json.dumps(result,indent=2))
bpy.ops.wm.save_as_mainfile(filepath=r'D:\AI\Mira3D\Mira2\mira2-grounded-rigged.blend')

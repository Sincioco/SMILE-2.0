...
bpy.ops.import_scene.fbx(filepath=r'D:\AI\Mira3D\Mira3\Mira3-HealOne-WithSkin.fbx')
rig=next(o for o in bpy.context.scene.objects if o.type=='ARMATURE');rig.name='Mira3.Rig'
body=next(o for o in bpy.context.scene.objects if o.type=='MESH');body.name='Mira3.SkinnedBody'
rig.animation_data.action.name='HealOne';rig.animation_data.action.use_fake_user=True
"""Assemble Mira3 using absolute Mixamo joint poses, then normalize OBJ/FBX units.
Run in the prepared Mira3 Blender scene containing Mira3.Rig and Mira3.SkinnedBody.
"""
import bpy,json
from pathlib import Path
root=Path(r'D:\AI\Mira3D\Mira3'); rig=bpy.data.objects['Mira3.Rig']; body=bpy.data.objects['Mira3.SkinnedBody'];scene=bpy.context.scene
...
bpy.ops.wm.save_as_mainfile(filepath=str(root/'mira3-assembled.blend'))
(root/'mira3-retarget-report.json').write_text(json.dumps({'unitMultiplier':100,'clips':report},indent=2))

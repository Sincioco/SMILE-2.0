"""Skin Yalis's reduced body and retarget her selected Mixamo animation set."""
...
CLIPS = {
    ...
    "Walk": ("Walk.fbx", "Sword And Shield Walk (In Place)"),
    "Run": ("Run.fbx", "Sword And Shield Run (In Place)"),
    "Attack": ("Attack.fbx", "Sword And Shield Cross Slash"),
    "Attack2": ("Attack2.fbx", "Sword And Shield Slash Combo"),
    "Dodge": ("Dodge.fbx", "Dodging Right"),
    "Defend": ("Defend.fbx", "Sword And Shield Block Idle"),
    "Hit": ("Hit.fbx", "Standing React Small From Front"),
    "Death": ("Death.fbx", "Standing React Death Backward"),
    "Victory": ("Victory.fbx", "Celebrating After A Win"),
}
...
def transfer_proxy_weights(proxy, body):
    ...
    transfer = body.modifiers.new(name="Yalis Proxy Weight Transfer", type="DATA_TRANSFER")
...
def parent_to_rig(body, rig):
    ...
    modifier = body.modifiers.new(name="Yalis Armature", type="ARMATURE")
    modifier.object = rig
...
def import_rig():
    ...
    bpy.ops.import_scene.fbx(
        filepath=str(MIXAMO / "yalis-v1-rigged-tpose.fbx"),
        use_anim=True,
    ...
    rig.name = "Yalis.Rig"
    proxy.name = "Yalis.RigProxy.Donor"
...
def retarget_clip(rig, clip_name, source_path, description):
    ...
    print("YALIS_CLIP_READY " + json.dumps(result), flush=True)
    return result


def main():
    bpy.ops.wm.open_mainfile(
        filepath=str(BLENDER / "yalis-v1-unrigged-reduced.blend")
    )
    scene = bpy.context.scene
    scene.render.fps = 30
    ...
    report = {
        "rigSource": "Mixamo auto-rigged clean Yalis-proportioned proxy",
        "skeletonPreset": "No Fingers (25 bones)",
    ...
    print("YALIS_ANIMATION_ASSEMBLED", flush=True)

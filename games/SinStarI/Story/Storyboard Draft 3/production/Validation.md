...
- 121 scene illustrations with matching silent LTX 2.5 previews in both views.
...
- One shared header, Script/Storyboard tabs, 53-scene sidebar, scene search,
  reading-position links, Print control and persistent light/dark themes.
...
- `site-validation.json`: three HTML pages; all local file/anchor references;
  ...
  all PNG files decoded; all three site JavaScript files syntax-checked.
- `media-validation.json`: every preview fully decoded with FFmpeg, checked for
  silent H.264/YUV420p video at 24 fps and 960-pixel width, and SHA-256 recorded.
  Ordinary previews run about four seconds; selected reused excerpts vary.
  The planet-throw preview runs about eight seconds.
- `behavior-validation.json`: 15 isolated playback checks and 17 isolated theme,
  navigation and search checks, run with the existing Node runtime and fake
  ...
  restrictions, current-scene transfer and cross-view theme links.

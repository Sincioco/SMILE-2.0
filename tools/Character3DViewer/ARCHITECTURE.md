...
Substantial implementation still in `Program.smile` after the playback state/lifecycle
...
ceiling at 2,782 lines/66 procedures. Its next concrete reductions are the 300-line
inspector command dispatcher, 83-line Party pointer dispatcher and 239-line load
...
`ViewerParty` remains a justified temporary exception at 2,104 lines/52 procedures: it
owns one Party scene's participant, preview, choreography, camera, presentation and
destruction lifecycle, and the seven-line glow transition added here belongs with that
state. The final responsibility audit must identify a cohesive lifecycle boundary before

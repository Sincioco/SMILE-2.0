...
Substantial implementation still in `Program.smile` after the Party preview/session-result
checkpoint is intentionally explicit: ordered load/retry/tab-switch orchestration; Party
inspector context/calibration adaptation and cross-owner Party reset/pointer response execution;
...
ceiling at 2,614 lines/59 procedures. Its next concrete reductions are the 274-line
inspector command dispatcher, 76-line Party pointer dispatcher and 223-line load
...
|---|---|---|---|---|---|
| Startup and failure/session lifecycle | `Program.smile` | `ViewerSession.smile` plus thin `Program.smile` coordinator | Running/readiness, first error/stage, resource epoch, tab/profile and scene mode | Select/skip/cycle profile, start/close, block/clear switch, capture epoch and reset/record/capture failure; coordinator borrows renderer and subsystem owners for ordered load/retry/switch/shutdown | One application session; direct module assertions plus native launch, reload and failure fixtures |

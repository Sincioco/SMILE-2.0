...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerUi` owns live label/layout policy plus status, camera, animation, calibration-panel and timeline drawing. `HandleInspectorPointer`, `HandleCalibrationPanelPointer`, `DrawInspectorOverlay`, edit-session commands and pointer action dispatch remain in `Program.smile`. | `ViewerCalibration.smile` for stored edit transactions and `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
Substantial implementation still in `Program.smile` after the calibration/timeline drawing checkpoint is
...
input routing, the overall inspector/error overlay and calibration/timeline pointer dispatch;
...
| UI and transform gizmo | `Program.smile` | `ViewerUi.smile` and `ViewerGizmo.smile` | Panel visibility, slider owner, opt-in gizmo projection/hover/drag state | UI label/geometry policy plus status/camera/animation/calibration/timeline drawing and gizmo projection/hit test/retained drag math/draw have moved; coordinator still owns the overall inspector/error overlay and pointer/edit command sequencing | Scene/edit; direct label/geometry/panel draw and gizmo hit-test/draw assertions plus pointer exclusivity and cancel tests |
...
| UI visibility, calibration panel/edit/confirmation fields, responsive geometry, status/camera/animation/calibration/timeline drawing and presentation labels in `Program.smile` | `ViewerUi.State` with explicit visibility, calibration reset, edit and confirmation transitions; narrow label/geometry operations plus focused panel renderers | State transitions, label/layout policy and focused control/panel renderers moved. Hit testing uses the same owner dimensions. The overall inspector/error overlay and pointer routing remain. Dead labels and unused slider calculations/constants were deleted. Pending edits and imports must continue to block actor switches without changing their owner. |

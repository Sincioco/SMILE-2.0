...
| Transform-gizmo command coordination | `ViewerGizmo` now owns projection, hit testing, retained ring-drag math, hover state and complete axis/ring drawing. `ViewerCalibrationEditing` owns calibration edit lifecycle, bounded mutations and grip preservation during gizmo edits. `Program.smile` retains keyboard/pointer action dispatch and timeline/capture ordering across the owners. | `ViewerInput.smile`, `ViewerGizmo.smile` and `ViewerCalibrationEditing.smile`, with cross-owner dispatch remaining in the coordinator |
...
Substantial implementation still in `Program.smile` after the Party companion-creation checkpoint is

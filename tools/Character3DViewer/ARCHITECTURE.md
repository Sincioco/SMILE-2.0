...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerUi` owns live label/layout policy, focused panel drawing, calibration hit classification/selection updates, timeline pointer-to-frame conversion and keyframe-marker hit testing. `HandleInspectorPointer`, the calibration action dispatcher, `DrawInspectorOverlay`, edit-session commands and broader pointer action dispatch remain in `Program.smile`. | `ViewerCalibration.smile` for stored edit transactions and `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
| General rendering and overlay composition | Socket resources and studio-grid wrappers reside in `ViewerRendering`; labels, responsive geometry, status/camera/animation controls, calibration panel and timeline reside in `ViewerUi`. `DrawViewerOverlay` and the overall inspector/error overlay remain in `Program.smile`. | `ViewerRendering.smile` for scene resources and `ViewerUi.smile` for editor presentation |
...
Substantial implementation still in `Program.smile` after the timeline pointer-classification checkpoint is
...
input routing, the overall inspector/error overlay and remaining inspector/timeline action dispatch;
...
| Calibration key banks, selection/edit/clipboard/Undo buffers, import baseline, target mapping/reset, wrist/equipment transforms and grip preservation in `Program.smile` | `ViewerCalibration.State`, module-private bounded workspaces, target operations, `ApplyWristOffsets`, `ApplyEquipmentCoupling`, `ApplyEquipmentTransforms` and `RestoreEquipmentGrip` | `Program.smile` coordinates UI-visible edit sessions and adapts explicit application results to session failure ownership; profile-scoped storage, transform/grip application, codec validation, rollback and import confirmation are direct production-module paths |
...
| Slider/timeline/repeat capture flags and ad hoc queued arrow checks in `Program.smile` | `ViewerInput.State`, `ClassifyArrow` and capture begin/finish/reset operations | State and low-level capture transitions moved; remaining inspector/timeline action dispatch still coordinates input with playback and calibration owners. Queued Ctrl is sampled from `Key_Event_Held`, and the invalid foreground-stealing `Window_Activate()` probe remains removed. |
| UI visibility, calibration panel/edit/confirmation fields, responsive geometry, calibration/timeline hit classification, status/camera/animation/calibration/timeline drawing and presentation labels in `Program.smile` | `ViewerUi.State` with explicit visibility, calibration reset, edit and confirmation transitions; narrow label/geometry/hit operations plus focused panel renderers | State transitions, label/layout policy, calibration hit/selection policy, timeline frame/keyframe hit math and focused renderers moved. The overall inspector/error overlay, general inspector classification and cross-owner action dispatch remain. Dead labels, unused slider calculations/constants and the redundant calibration frame-time wrapper were deleted. Pending edits and imports must continue to block actor switches without changing their owner. |

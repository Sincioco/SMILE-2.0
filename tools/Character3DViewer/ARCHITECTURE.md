...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist-socket discovery/availability, edit bounds, key mutation/persistence transactions, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerUi` owns live label/layout policy, slider mapping, focused panel drawing, calibration/timeline/general inspector hit classification and raw presentation leaves. `HandleInspectorPointer`, UI confirmation and edit-session commands remain in `Program.smile` as cross-owner command/capture/application sequencing; `DrawInspectorOverlay` remains only as the explicit cross-owner draw coordinator. | `ViewerCalibration.smile` for stored transactions and `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
Substantial implementation still in `Program.smile` after the calibration key-transaction checkpoint is
...
| `CalibrationTests.smile` plus generated isolated project | `ViewerCalibration`, `ViewerParty`, `ViewerEffects`, `ViewerRendering` and retained `CalibrationJson` | Production owners are included directly; move/delete/paste/reload/clear, failed persistence, primary/backup recovery, import confirmation and Undo exercise `ViewerCalibration` without coordinator query/storage wrappers. Native/generated-Web tests also prove scene pause leaves Fire and Lightning ages advancing by default, while explicit family toggles remain independent. |

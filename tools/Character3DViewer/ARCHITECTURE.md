...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerUi` owns live label policy, shared responsive geometry, minimum-size policy, status summary, camera sliders and animation buttons. `HandleInspectorPointer`, `HandleCalibrationPanelPointer`, `DrawInspectorOverlay`, `DrawCalibrationPanel`, `DrawTimeline`, edit-session commands and remaining layout/drawing routines remain in `Program.smile`. | `ViewerCalibration.smile` for stored edit transactions and `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
| General rendering and overlay composition | Socket resources and studio-grid wrappers reside in `ViewerRendering`; reusable label policy resides in `ViewerUi`. `DrawViewerOverlay`, inspector/calibration/timeline drawing and related layout remain in `Program.smile`. | `ViewerRendering.smile` for scene resources and `ViewerUi.smile` for editor presentation |
...
Substantial implementation still in `Program.smile` after the UI control-drawing checkpoint is
...
input routing, overall inspector overlay, calibration panel and timeline drawing;
...
| UI and transform gizmo | `Program.smile` | `ViewerUi.smile` and `ViewerGizmo.smile` | Panel visibility, slider owner, opt-in gizmo projection/hover/drag state | UI label/geometry policy plus status/camera/animation-control drawing and gizmo projection/hit test/retained drag math/draw have moved; coordinator still owns the overall inspector, calibration/timeline panels and command sequencing | Scene/edit; direct label/geometry/draw and gizmo hit-test/draw assertions plus pointer exclusivity and cancel tests |
...
| UI visibility, calibration panel/edit/confirmation fields, responsive geometry, status/camera/animation-control drawing and presentation labels in `Program.smile` | `ViewerUi.State` with explicit visibility, calibration reset, edit and confirmation transitions; narrow label/geometry operations plus `DrawCharacterStatusSummary`, `DrawCameraControls` and `DrawAnimationButtons` | State transitions, live label policy and these focused control-drawing implementations moved. Hit testing uses the same owner dimensions. The overall inspector overlay, calibration/timeline panels and pointer routing remain. Three unused label routines, unused slider calculations and four unused slider constants were deleted. Pending edits and imports must continue to block actor switches without changing their owner. |

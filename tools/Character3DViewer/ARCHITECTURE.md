...
Substantial implementation still in `Program.smile` after the inspector display-capture
...
executor. The retained calibration adapters enter/restore Party preview, request
...
ceiling at 2,007 lines/56 procedures. The substantial routines still requiring ownership
judgment are the 213-line inspector pointer dispatcher, 77-line inspector overlay,
77-line interface pointer coordinator, 76-line
...
procedures and retains no application state. `ViewerLifecycle` is 622 lines/22 procedures and is explicitly reviewed above
...
|---|---:|---|
| `HandleInspectorPointer` | 213 | Preserves the ordered timeline, tab, transfer, calibration, inspector and scene-action priority while sampling runtime pointer state. State-local action behavior and application-response policy have moved; runtime gesture/transfer/timeline orchestration remains pending. |
| `DrawInspectorOverlay` | 77 | Preserves full-screen/minimum-size gates, Party overlay placement and identity/status/effects/workspace group order. Scalar-only display capture and raw drawing live in the stateless presentation owner; this is a justified cross-owner draw coordinator. |
...
`HandleInspectorKeyboard` is now a 60-line threshold-boundary coordinator: it samples
queued modifier state, preserves gizmo-first priority and calls focused command owners.
`ApplyInspectorApplicationCommand` is the single 25-line visible executor shared by pointer

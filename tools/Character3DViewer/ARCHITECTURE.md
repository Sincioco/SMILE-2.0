...
|---|---|---|
| Party transitions, companion lifecycle and inspector binding | `ViewerParty.AdvanceChoreography`, `ApplyFrame`, `ApplyAttackCamera`, participant update/placement/drawing, preview operations, pointer classification, overlay drawing and destruction now own their implementations. `ViewerActors.FaceToward` owns reusable facing and `ViewerEffects` owns borrowed companion glow creation/update. `Program.smile` retains companion creation/calibration setup, inspector target selection, formation/reset commands, narrow session-failure adaptation and pointer action dispatch. | `ViewerParty.smile`, `ViewerActors.smile` and `ViewerEffects.smile`, with cross-owner creation/selection remaining in the coordinator |
...
Substantial implementation still in `Program.smile` after the calibration edit-session checkpoint is
intentionally explicit: startup/retry/tab-switch orchestration; Party
inspector selection, companion creation/calibration setup,
...
| Party participants, inspector/preview binding, turn/stage/timing, attack/reaction state, Party battle cameras, pointer hit map, calibrated companion updates, participant placement/drawing/destruction and Party overlay implementation in `Program.smile` | `ViewerParty.State`, two explicit `ParticipantLayout` values, choreography/preview operations, `ApplyFrame`, `UpdateCompanion`, `UpdateDragonOpponent`, `PlaceDragonInspectionParticipants`, `ApplyAttackCamera`, `ClassifyPointer`, participant drawing, `DrawOverlay` and `DestroyParticipants` | State, choreography, preview playback, actor command/calibration/effect application, camera construction/continuity, pointer classification, drawing/destruction and overlay rendering moved with direct tests and static ownership contracts. The coordinator retains pointer action dispatch, companion creation/calibration setup, formation/reset commands, session-failure adaptation and inspector target selection. Exactly two live Party participants remain explicit and same-model fallback must not alias actor state. |
...
- Party, effects and rendering state plus selected calculations moved into focused
  ...
  still contains the substantial companion creation/calibration setup, inspector
  selection and Party command dispatch listed above; calibrated participant update now
  resides in `ViewerParty`. Transform-gizmo hit testing, retained drag math and drawing

...
Substantial implementation still in `Program.smile` after the lifecycle-coordinator
...
ceiling at 2,276 lines/56 procedures. The substantial routines still requiring ownership
judgment are the 274-line inspector pointer dispatcher, 171-line inspector overlay,
...
|---|---:|---|
| `HandleInspectorPointer` | 274 | Preserves the ordered timeline, tab, transfer, calibration, inspector and scene-action priority while sampling runtime pointer state. Named response dispatch remains substantial and is the next owner-level extraction. |

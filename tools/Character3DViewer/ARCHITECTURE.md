...
ceiling at 2,037 lines/55 procedures. The substantial routines still requiring ownership
judgment are the 227-line inspector pointer dispatcher, 77-line inspector overlay,
...
`ViewerInspectorCommands` is 397 lines/7 procedures,
...
|---|---:|---|
| `HandleInspectorPointer` | 227 | Preserves the ordered timeline, tab, transfer, calibration, inspector and scene-action priority while sampling runtime pointer state. State-local action behavior and response policy have moved; runtime gesture/transfer/timeline orchestration remains pending. |
...
| `HandleInspectorKeyboard` | 104 | Samples queued key state and preserves gizmo-first plus inspector action priority. Owner-local commands have moved, but the remaining response dispatcher is pending. |

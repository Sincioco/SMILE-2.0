...
|---|---|---|
| Party transitions, companion lifecycle and inspector binding | `ViewerParty.CreateCompanion`, `ResetDemo`, `AdvanceDemo`, lower-level choreography/frame operations, `ApplyAttackCamera`, participant update/placement/drawing, preview operations, pointer classification, overlay drawing and destruction own their implementations. `ViewerActors` owns actor loading/facing and `ViewerEffects` owns shared and borrowed glow resources. `Program.smile` retains inspector target selection, narrow session-readiness adaptation and pointer action dispatch. | `ViewerParty.smile`, `ViewerActors.smile` and `ViewerEffects.smile`, with cross-owner inspector selection remaining in the coordinator |
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist-socket discovery/availability, edit bounds, key navigation/mutation/persistence transactions, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerCalibrationEditing` owns current evaluation/value access plus edit begin/finish/cancel, value/reset, grip correction and transform-application sequencing. `ViewerPlayback` owns clip mapping/labels/events, playback-state mutation, frame/time seeking and demo advancement. `ViewerInput` owns inspector key policy and `ViewerUi` owns live label/layout policy, slider mapping, focused panel drawing, calibration/timeline/general inspector hit classification and raw presentation leaves. Pointer/persistent command dispatch, Party-preview/session adaptation and timeline gesture dispatch remain in `Program.smile`; `DrawInspectorOverlay` remains only as the explicit cross-owner draw coordinator. | `ViewerCalibration.smile` and `ViewerCalibrationEditing.smile` for stored/edit behavior; `ViewerPlayback.smile` for playback; `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
Substantial implementation still in `Program.smile` after the profile/arena-policy checkpoint is
intentionally explicit: startup/retry/tab-switch orchestration; Party
inspector selection and pointer action dispatch;
input capture sequencing and the large inspector/keyboard/timeline/gizmo action dispatchers;
calibration import/save/copy/paste/Undo confirmation and persistent-command ordering;
reset orchestration and cross-owner overlay/gizmo composition.
The narrow calibration edit wrappers remain because they enter/restore Party preview or
adapt an explicit operation result to first-failure session state; the edit implementation
is in `ViewerCalibrationEditing`. `UpdateDragon` remains because the coordinator
...
| Actor/inspector binding | `Program.smile` (`ViewerActorContext`, `PartyUi*`, `CalibrationOwnerProfile`) | `ViewerActors.smile` owns `Context`, generic actor load/update/draw/destroy, loaded profile validation and arena-facing policy; `ViewerParty.smile` owns companion creation and preview state/mode/restore; inspector target and calibration-owner selection remain in the coordinator | Primary/inspected actor identity, Party participants and temporary preview binding | Participant creation combines the actor, calibration and glow owners through explicit borrowed state; startup/reset pass explicit Dragon/profile constants to actor-facing policy; cross-owner inspector selection still borrows the calibration owner from `Program.smile` | Scene/preview; direct context/preview/policy assertions plus integrated Party/Dragon load and isolation fixtures |
| Camera and transforms | `Program.smile`, `BattleCamera.smile`, shared `Interaction` | `ViewerCamera.smile` and retained `BattleCamera.smile` math | `ViewerCamera.State`: base/live camera, frame, controls, zoom target, fractional pointer remainder, orbit anchor and auto-orbit | Reset/compose/nudge/drag/advance/apply/query plus character-versus-arena orbit/zoom default selection; borrows framing profile and current actor bounds without retaining either | Scene; direct integer-output/default-policy assertions plus native/installed-Chrome controls |
| Calibration editing | `Program.smile`, `CalibrationJson.smile` | `ViewerCalibration.smile`, `ViewerCalibrationEditing.smile` and retained bounded JSON reader | `ViewerCalibration` owns per-profile key banks and edit/clipboard/Undo/import workspace; UI, gizmo, camera and effects retain their own interaction state | Stored operations configure/load/evaluate/apply/map/reset/preserve/save/Undo/import/export/query; the edit-session owner coordinates current evaluation/value, begin/finish/cancel, bounded set/reset, grip correction and transform application through explicit borrowed states | Profile/edit workspace; direct owner checks, native/generated-Web isolation, real actor transform/grip integration and malformed imports |
...
| Input ownership | `Program.smile` | `ViewerInput.smile` | Pointer capture, timeline/frame repeat and inspector key classification | Classify queued keyboard policy and own capture transitions; the coordinator dispatches returned actions while borrowing UI, calibration and camera operations | Frame/capture; direct editing/Party/queued-modifier/socket/clip policy and outside-window fixtures |
| UI and transform gizmo | `Program.smile` | `ViewerUi.smile`, `ViewerGizmo.smile` and `ViewerCalibrationEditing.smile` | Panel visibility and slider owner remain UI/input state; opt-in projection, hover, drag and retained fractional motion remain gizmo state | UI label/geometry, calibration hit/selection policy and focused panels have moved; gizmo target origin, pointer projection, retained value conversion, hit testing and drawing are production-owner operations. The coordinator still owns the overall inspector/error overlay and cross-owner pointer/edit command dispatch | Scene/edit; direct label/geometry/panel/hit assertions plus gizmo remainder/hit-test/draw, pointer exclusivity and cancel integration |
...
| Rendering and overlay composition | `Program.smile` | `ViewerRendering.smile` for scene resources and Dragon-aware floor extent policy; `ViewerUi.smile` for editor overlays | `ViewerRendering.State` owns arena/backdrop/grid/socket render resources including the fixed socket-object array; UI retains transient layout state | Create/update/draw/destroy scene resources and resolve explicit character/arena extents; overlays borrow read-only snapshots from owners | Scene; direct extent/socket selection/part-routing assertions plus native/generated-Web draw and resize checks |
...
| `SelectedClip`, speed, pause/demo counters, clip mapping/labels/events, frame/time seek and play/demo sequence calculations | `ViewerPlayback.State`, `SelectClip`, `StepFrame`, `SeekFirstFrame`, `SeekTime`, `StartSelectedClip`, `AdvanceDemo`, mapping/query operations and `Demo*` | Playback state and behavior move together; actor/profile handles are borrowed per call and never retained. Party preview, calibration/effect application and timeline gesture order remain explicit coordinator work. |
...
| Calibration key banks, selection/edit/clipboard/Undo buffers, import baseline, target mapping/reset, wrist discovery, value bounds, wrist/equipment transforms, grip preservation and edit-session routines in `Program.smile` | `ViewerCalibration.State`, module-private bounded workspaces and stored operations plus `ViewerCalibrationEditing.EvaluateCurrent`, `CurrentValue`, `BeginEdit`, `FinishEdit`, `CancelEdit`, `SetCurrentValue` and `ResetTarget` | Actor capability discovery, profile-scoped storage, key transactions, transform/grip application, codec validation, rollback and import confirmation are direct production-module paths. `Program.smile` retains Party-preview entry, persistent-command ordering and explicit session-failure adaptation; it no longer owns edit lifecycle or grip implementation. |
...
| Slider/timeline/repeat capture flags, ad hoc queued arrow checks and inspector key policy in `Program.smile` | `ViewerInput.State`, `ClassifyArrow`, `ClassifyInspectorKey` and capture begin/finish/reset operations | State, low-level capture transitions and key-to-action policy moved; remaining inspector/timeline action dispatch coordinates input with playback and calibration owners. General hit geometry comes from `ViewerUi`. Queued Ctrl is sampled once from `Key_Event_Held` and passed explicitly, and the invalid foreground-stealing `Window_Activate()` probe remains removed. |
...
| Transform-gizmo target origin, projection, pointer ownership, drag remainder, hit testing, ring/move pointer conversion, drawing and grip state in `Program.smile` | `ViewerCalibrationEditing.GizmoOrigin` plus `ViewerGizmo.State` with reset/show-hide/begin/finish/select/projection/hit-test/ring-delta/retained-value/draw operations | Target/socket mapping resides with calibration editing; projection and fractional drag behavior reside with gizmo state. The coordinator retains pointer sampling and calibration command sequencing only. Gizmos remain opt-in and hiding retains the unsaved numeric preview for explicit Save or Cancel. |
| Party participants, inspector/preview binding, turn/stage/timing, attack/reaction state, Party battle cameras, pointer hit map, companion creation/calibrated updates, participant placement/drawing/destruction and Party overlay implementation in `Program.smile` | `ViewerParty.State`, two explicit `ParticipantLayout` values, `CreateCompanion`, `ResetDemo`, `AdvanceDemo`, choreography/preview operations, `ApplyFrame`, participant updates, placement, `ApplyAttackCamera`, `ClassifyPointer`, drawing, overlay and destruction | Participant load/calibration/glow/playback, reset/formation/turn preparation, per-frame choreography/commands, preview playback, calibrated updates, camera construction/continuity, pointer classification, drawing/destruction and overlay rendering moved with integration/direct tests and static ownership contracts. The coordinator retains pointer action dispatch, session-failure adaptation and inspector target selection. Exactly two live Party participants remain explicit and same-model fallback must not alias actor state. |
...
|---|---|---|
| `HardeningTests.smile` session/playback/clock/zoom/camera/input/UI/gizmo/Party/effects/rendering assertions | `ViewerSession`, `ViewerPlayback`, `ViewerTiming`, `ViewerCamera`, `ViewerCalibrationEditing`, `ViewerInput`, `ViewerUi`, `ViewerGizmo`, `ViewerActors`, `ViewerParty`, `ViewerEffects`, `ViewerRendering`, `BattleCamera`, shared `CharacterViewer` | New owners are included and called directly; playback selection state, inspector editing/Party/queued-Ctrl/socket/clip keyboard policy, exclusive capture, current calibration-value and inactive-edit behavior, UI presentation labels, exact responsive/calibration/timeline geometry, calibration pointer boundaries/selection and slider endpoints, timeline endpoint/key hits, tab/transfer/panel/scene/profile/animation hit-map priority, unavailable actor/socket discovery and rotation/position bounds, all inspector presentation leaves, opt-in gizmo retained-drag/hide/hit-test/draw, camera cancellation, Party reset/formation/binding/timing/preview policy/frame application/labels/pointer classification/battle-camera selection, actor facing, independent VFX pause, socket display/selection/part routing and rendering resource transitions exercise production code. Static contracts keep moved playback queries/algorithms, input key parsing, calibration discovery/bounds/edit-session/origin implementation, UI drawing/constants/text and all UI hit geometry, gizmo drag/draw implementation, Party reset/advance/actor-command/presentation/camera implementation and socket/grid resources out of `Program.smile`. |
| `CalibrationTests.smile` plus generated isolated project | `ViewerPlayback`, `ViewerCalibration`, `ViewerCalibrationEditing`, `ViewerParty`, `ViewerEffects`, `ViewerRendering` and retained `CalibrationJson` | Real-actor mapping/name/start/frame-seek tests exercise `ViewerPlayback`; adjacent-key navigation and move/delete/paste/reload/clear, failed persistence, primary/backup recovery, import confirmation and Undo exercise `ViewerCalibration` without coordinator query/storage wrappers. Current-value queries use the edit-session production owner, while existing set/cancel/reload and repeated grip-rotation integration continues through the startup command boundary. Party and Dragon-inspection loads call `ViewerParty.CreateCompanion`; assertions prove Arin/Orin identity, calibration availability and primary-bank restoration before a real calibrated companion update. The direct no-op update check distinguishes skipped from attempted updates. Native/generated-Web tests also prove scene pause leaves Fire and Lightning ages advancing by default, while explicit family toggles remain independent. |

...
Substantial implementation still in `Program.smile` after the general inspector-pointer
...
dispatchers plus interface/slider/gizmo/camera runtime sampling and priority ordering;
...
ceiling at 2,229 lines/56 procedures. The substantial routines still requiring ownership
...
orchestration and 71-line camera pointer sampler. The retained `DrawInspectorOverlay`
...
`ViewerCamera` is 608 lines/32 procedures, `ViewerGizmo` is 620/18 and
...
| `HandleInterfacePointer` | 77 | Preserves timeline release, outside-window capture recovery and inspector-before-Party-before-camera pointer ownership. The runtime sampling boundary remains pending review. |
| `HandlePartyPointer` | 76 | Applies the Party owner's typed pointer decision and reset/speed/pause response in visible order. The response execution boundary remains pending. |
...
| `UpdateCameraPointer` | 71 | Samples runtime pointer/wheel state after interface ownership and forwards explicit values to `ViewerCamera`. Camera policy and state already moved; the sampling adapter remains pending review. |

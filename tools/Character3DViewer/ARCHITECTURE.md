...
Substantial implementation still in `Program.smile` after the Party demo-lifecycle checkpoint is
...
profile/arena-facing policy, reset orchestration, and cross-owner overlay/gizmo composition.
...
| Actor/inspector binding | `Program.smile` (`ViewerActorContext`, `PartyUi*`, `CalibrationOwnerProfile`) | `ViewerActors.smile` owns `Context` and generic actor load/update/draw/destroy/facing; `ViewerParty.smile` owns companion creation and preview state/mode/restore; inspector target and calibration-owner selection remain in the coordinator | Primary/inspected actor identity, Party participants and temporary preview binding | Participant creation combines the actor, calibration and glow owners through explicit borrowed state; cross-owner inspector selection still borrows the calibration owner from `Program.smile` | Scene/preview; direct context/preview assertions plus integrated Party/Dragon load and isolation fixtures |

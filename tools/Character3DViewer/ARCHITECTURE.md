...
Substantial implementation still in `Program.smile` after the presentation audio/storm checkpoint is
...
The narrow calibration command/edit wrappers remain because they enter/restore Party preview,
apply scene pause/demo policy or adapt an explicit operation result to first-failure
session state; transaction and edit implementation is in `ViewerCalibrationEditing`.
...
| Effects/audio | `Program.smile`, `OrinStorm.smile`, `DragonPresence.smile`, `BattleAudio.smile` | `ViewerEffects.smile` plus retained focused modules; `ViewerParty.smile` selects the Orin presentation actor/target | Equipment emitters, trails, leases, scene clocks, Arin cue state and visual-continuity epochs; the Dragon owner retains its own cue state and borrows its scene light lease | Create/update/advance-once/draw/invalidate/destroy, Arin cue update/reset and Party Orin routing; borrows final actor transforms | Scene; direct control/state assertions plus frozen-cut/skipped-cue/storm/lease cleanup tests |

...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerUi` now owns the live presentation-label policy. `HandleInspectorPointer`, `HandleCalibrationPanelPointer`, `DrawInspectorOverlay`, `DrawCalibrationPanel`, `DrawTimeline`, edit-session commands and layout/drawing routines remain in `Program.smile`. | `ViewerCalibration.smile` for stored edit transactions and `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
Substantial implementation still in `Program.smile` after the UI presentation-policy checkpoint is
...
input routing and UI layout/drawing; calibration edit command coordination and
...
| UI and transform gizmo | `Program.smile` | `ViewerUi.smile` and `ViewerGizmo.smile` | Panel visibility, slider owner, opt-in gizmo projection/hover/drag state | UI label policy and gizmo projection/hit test/retained drag math/draw have moved; coordinator still owns layout, panel drawing and calibration command sequencing | Scene/edit; direct label and gizmo hit-test/draw assertions plus pointer exclusivity and cancel tests |
...
| UI visibility, calibration panel/edit/confirmation fields and presentation labels in `Program.smile` | `ViewerUi.State` with explicit visibility, calibration reset, edit and confirmation transitions; narrow label functions for playback, calibration, background, Dragon, material, demo and Party role | State transitions and live label policy moved. Substantial layout, panel/overlay drawing and pointer routing remain for R7.5. Three unused label routines were deleted. Pending edits and imports must continue to block actor switches without changing their owner. |

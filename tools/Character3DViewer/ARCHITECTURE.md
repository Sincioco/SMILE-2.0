...
| Inspector, calibration panel, timeline, camera controls, buttons and overlays | `ViewerCalibration` owns stored calibration evaluation, wrist-socket discovery/availability, edit bounds, key navigation/mutation/persistence transactions, wrist/equipment transform application, target/channel mapping/reset and inverse grip preservation. `ViewerPlayback` owns clip mapping/labels/events, playback-state mutation, frame/time seeking and demo advancement. `ViewerUi` owns live label/layout policy, slider mapping, focused panel drawing, calibration/timeline/general inspector hit classification and raw presentation leaves. `HandleInspectorPointer`, UI confirmation, edit-session commands and timeline gesture sequencing remain in `Program.smile` as cross-owner coordination; `DrawInspectorOverlay` remains only as the explicit cross-owner draw coordinator. | `ViewerCalibration.smile` and `ViewerPlayback.smile` for stored/playback behavior; `ViewerInput.smile`/`ViewerUi.smile` for interaction and presentation |
...
Substantial implementation still in `Program.smile` after the calibrated Party-participant checkpoint is
...
input capture sequencing and inspector/timeline gesture dispatch;
calibration UI confirmation, edit-gesture and transform/effect application coordination;

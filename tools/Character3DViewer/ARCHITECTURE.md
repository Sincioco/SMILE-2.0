...
Substantial implementation still in `Program.smile` after the interaction-lifecycle
...
and cross-owner scene, Party and gizmo drawing/order. Inspector display capture and
...
ceiling at 1,967 lines/53 procedures. The substantial routines still requiring ownership
...
`ViewerRendering` is 643/30. Each was reviewed above the 500-line threshold and remains
below the 800-line ceiling with one focused state/lifecycle responsibility.
...
| Renderer load/cache/shadow policy, arena, floor/grid visibility, backdrop handles/index, lighting/material modes, socket object array, marker resources, display selection and renderer-mode/socket lifecycle routines in `Program.smile` | `ViewerRendering.State` with `PrepareAssetLoading`, `ConfigureShadowArea`, arena/backdrop operations, `ResetControls`, lighting/material operations, socket-gizmo operations, selection and destruction; `ViewerInspectorCommands` for the rendering action map | Scene-resource, load policy, renderer-mode state, implementation and lifecycle now move together. The coordinator retains original load/reset/update/draw ordering and explicit readiness accumulation; a later successful quality call cannot erase an earlier failure. Rendering does not own gameplay, Party, calibration or session state. |

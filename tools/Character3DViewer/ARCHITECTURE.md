...
|---|---|---|
| Party transitions, companion lifecycle and inspector binding | `ViewerParty.CreateCompanion`, `AdvanceChoreography`, `ApplyFrame`, `ApplyAttackCamera`, participant update/placement/drawing, preview operations, pointer classification, overlay drawing and destruction own their implementations. `ViewerActors` owns actor loading/facing and `ViewerEffects` owns shared and borrowed glow resources. `Program.smile` retains inspector target selection, formation/reset commands, narrow session-failure adaptation and pointer action dispatch. | `ViewerParty.smile`, `ViewerActors.smile` and `ViewerEffects.smile`, with cross-owner inspector selection remaining in the coordinator |
...
Substantial implementation still in `Program.smile` after the inspector keyboard-policy checkpoint is
intentionally explicit: startup/retry/tab-switch orchestration; Party
inspector selection, formation/reset commands and pointer action dispatch;
...
| Party participants, inspector/preview binding, turn/stage/timing, attack/reaction state, Party battle cameras, pointer hit map, companion creation/calibrated updates, participant placement/drawing/destruction and Party overlay implementation in `Program.smile` | `ViewerParty.State`, two explicit `ParticipantLayout` values, `CreateCompanion`, choreography/preview operations, `ApplyFrame`, `UpdateCompanion`, `UpdateDragonOpponent`, `PlaceDragonInspectionParticipants`, `ApplyAttackCamera`, `ClassifyPointer`, participant drawing, `DrawOverlay` and `DestroyParticipants` | Participant load/calibration/glow/playback, state, choreography, preview playback, actor command/calibration/effect application, camera construction/continuity, pointer classification, drawing/destruction and overlay rendering moved with integration/direct tests and static ownership contracts. The coordinator retains pointer action dispatch, formation/reset commands, session-failure adaptation and inspector target selection. Exactly two live Party participants remain explicit and same-model fallback must not alias actor state. |

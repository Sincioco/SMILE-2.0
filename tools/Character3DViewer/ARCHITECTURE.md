...
`Program.smile`. The bounded stage must not
...
Substantial implementation still in `Program.smile` after the Party actor/preview checkpoint is
...
- Party, effects and rendering state plus selected calculations moved into focused
  ...
  companion update and inspector-selection routines listed above.

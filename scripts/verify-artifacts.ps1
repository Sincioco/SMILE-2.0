...
try {
    ...
    if ($vsixManifest -notmatch 'Version="2\.0\.31"') {
        throw 'VSIX identity version is not 2.0.31.'
    }
}
finally {
    $archive.Dispose()
...
if ($versionInfo.FileVersion -ne '2.0.31.0' -or $versionInfo.ProductVersion -notlike '2.0.31*' -or
    $assemblyVersion -ne '2.0.31.0') {
...
Write-Host 'VSIX identity, assembly, file, and product versions are synchronized at 2.0.31.'

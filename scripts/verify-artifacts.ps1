...
try {
    ...
    foreach ($templateName in @('SmileConsole', 'SmileGame')) {
        ...
        $templateText -notmatch 'Version=2\.0\.63\.0') {
    ...
    if ($vsixManifest -notmatch 'Version="2\.0\.63"') {
        throw 'VSIX identity version is not 2.0.63.'
    }
...
if ($versionInfo.FileVersion -ne '2.0.63.0' -or $versionInfo.ProductVersion -notlike '2.0.63*' -or
    $assemblyVersion -ne '2.0.63.0') {
...
Write-Host 'VSIX identity, assembly, file, and product versions are synchronized at 2.0.63.'

...
try {
    ...
    foreach ($templateName in @('SmileConsole', 'SmileGame')) {
        ...
        $templateText -notmatch 'Version=2\.0\.52\.0') {
    ...
    if ($vsixManifest -notmatch 'Version="2\.0\.52"') {
        throw 'VSIX identity version is not 2.0.52.'
    }
    if ($vsixManifest -notmatch 'Type="Microsoft\.VisualStudio\.Assembly"' -or
        $vsixManifest -notmatch 'AssemblyName="Smile\.VisualStudio, Version=2\.0\.52\.0, Culture=neutral, PublicKeyToken=null"') {
...
if ($versionInfo.FileVersion -ne '2.0.52.0' -or $versionInfo.ProductVersion -notlike '2.0.52*' -or
    $assemblyVersion -ne '2.0.52.0') {
...
Write-Host 'VSIX identity, assembly, file, and product versions are synchronized at 2.0.52.'

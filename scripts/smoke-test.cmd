...
if /i "%~1"=="--skip-doctor" goto doctor_complete
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-startup.ps1"
if errorlevel 1 exit /b %errorlevel%

powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-renderer3d-post-processing.ps1"
if errorlevel 1 exit /b %errorlevel%

powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-renderer3d-reflections.ps1"
if errorlevel 1 exit /b %errorlevel%
...
if errorlevel 1 exit /b %errorlevel%
powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-renderer3d-gpu-particle-webgl2.ps1"
if errorlevel 1 exit /b %errorlevel%

powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-model3d-asset-cooking.ps1"
if errorlevel 1 exit /b %errorlevel%

powershell -NoProfile -ExecutionPolicy Bypass -File "%SMILE_ROOT%\scripts\test-model3d-cooking-screenshot-evidence.ps1"
if errorlevel 1 exit /b %errorlevel%
...
powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip=[IO.Compression.ZipFile]::OpenRead('%SMILE_ROOT%\artifacts\libraries\Smile.Math.Extras.smilelib'); try { $manifest=([IO.StreamReader]::new($zip.GetEntry('manifest.json').Open())).ReadToEnd() | ConvertFrom-Json; $api=([IO.StreamReader]::new($zip.GetEntry('api/public-symbols.json').Open())).ReadToEnd() | ConvertFrom-Json; if ($manifest.formatVersion -ne 7 -or $api.formatVersion -ne 7 -or $manifest.name -ne 'Smile.Math.Extras' -or $manifest.version -ne '1.0.0' -or $manifest.provider -ne 'Smile.Math.Extras@1.0.0' -or @($manifest.modules).Count -ne 1 -or @($manifest.sources).Count -ne 2 -or @($manifest.dependencies).Count -ne 0 -or $api.library.provider -ne $manifest.provider) { exit 1 } } finally { $zip.Dispose() }"
if errorlevel 1 exit /b 1
...
if errorlevel 1 exit /b 1
powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip=[IO.Compression.ZipFile]::OpenRead('%SMILE_ROOT%\artifacts\libraries\Smile.Text.Extras.smilelib'); try { $manifest=([IO.StreamReader]::new($zip.GetEntry('manifest.json').Open())).ReadToEnd() | ConvertFrom-Json; $apiText=[IO.StreamReader]::new($zip.GetEntry('api/public-symbols.json').Open()).ReadToEnd(); $api=$apiText | ConvertFrom-Json; if ($manifest.formatVersion -ne 7 -or $api.formatVersion -ne 7 -or $manifest.name -ne 'Smile.Text.Extras' -or $manifest.version -ne '1.0.0' -or $manifest.provider -ne 'Smile.Text.Extras@1.0.0' -or @($manifest.modules).Count -ne 1 -or @($manifest.sources).Count -ne 1 -or @($manifest.dependencies).Count -ne 0 -or $api.library.provider -ne $manifest.provider -or !$apiText.Contains('ByRef') -or !$apiText.Contains('returnType') -or !$apiText.Contains('Text')) { exit 1 } } finally { $zip.Dispose() }"
if errorlevel 1 exit /b 1
...
powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip=[IO.Compression.ZipFile]::OpenRead('%SMILE_ROOT%\artifacts\libraries\Smile.Data.Models.smilelib'); try { $manifest=([IO.StreamReader]::new($zip.GetEntry('manifest.json').Open())).ReadToEnd() | ConvertFrom-Json; $apiText=[IO.StreamReader]::new($zip.GetEntry('api/public-symbols.json').Open()).ReadToEnd(); $api=$apiText | ConvertFrom-Json; if ($manifest.formatVersion -ne 7 -or $api.formatVersion -ne 7 -or $manifest.name -ne 'Smile.Data.Models' -or $manifest.version -ne '1.0.0' -or $manifest.provider -ne 'Smile.Data.Models@1.0.0' -or @($manifest.modules).Count -ne 1 -or @($manifest.sources).Count -ne 2 -or @($manifest.dependencies).Count -ne 0 -or $api.library.provider -ne $manifest.provider -or !$apiText.Contains('Smile.Data.Models::Actor') -or !$apiText.Contains('\"fields\"') -or $apiText.Contains('InternalTag')) { exit 1 } } finally { $zip.Dispose() }"
if errorlevel 1 exit /b 1
...
if errorlevel 1 exit /b 1
powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip=[IO.Compression.ZipFile]::OpenRead('%SMILE_ROOT%\artifacts\libraries\Smile.Game.smilelib'); try { $manifest=([IO.StreamReader]::new($zip.GetEntry('manifest.json').Open())).ReadToEnd() | ConvertFrom-Json; $apiText=([IO.StreamReader]::new($zip.GetEntry('api/public-symbols.json').Open())).ReadToEnd(); $api=$apiText | ConvertFrom-Json; $names=@($api.modules.name); $core=$api.modules | Where-Object name -eq 'Smile.Game.Core'; $cameraModule=$api.modules | Where-Object name -eq 'Smile.Game.Camera2D'; $direction=$core.members | Where-Object name -eq 'CardinalDirection'; $mover=$core.members | Where-Object name -eq 'CardinalMover'; $camera=$cameraModule.members | Where-Object name -eq 'CameraState'; $directionX=$core.members | Where-Object name -eq 'DirectionX'; $directionY=$core.members | Where-Object name -eq 'DirectionY'; $legacy=@($core.members | Where-Object { $_.name -in @('Place','BeginMove','UpdateMove','CancelMove','VisualX','VisualY') }); if ($manifest.formatVersion -ne 7 -or $api.formatVersion -ne 7 -or $manifest.provider -cne 'Smile.Game@2.0.0' -or $api.library.provider -cne $manifest.provider -or $manifest.version -ne '2.0.0' -or $names.Count -ne 5 -or $names -notcontains 'Smile.Game.Core' -or $names -notcontains 'Smile.Game.TileMap' -or @($api.modules.members).Count -ne 56 -or @($api.modules.members | Where-Object requiresGameWindow).Count -ne 2 -or (@($direction.members.name) -join '|') -cne 'None|Up|Right|Down|Left' -or (@($direction.members.value) -join '|') -cne '0|1|2|3|4' -or $direction.provider -cne $manifest.provider -or (@($mover.members.name) -join '|') -cne 'BeginMove|CancelMove|Place|UpdateMove|VisualX|VisualY' -or $mover.fields[8].name -cne 'Facing' -or $mover.fields[8].type.identity -cne 'Smile.Game.Core::CardinalDirection' -or $mover.fields[8].type.provider -cne $manifest.provider -or $mover.members[0].parameters[0].type.identity -cne 'Smile.Game.Core::CardinalDirection' -or $directionX.parameters[0].type.kind -cne 'enum' -or $directionY.parameters[0].type.provider -cne $manifest.provider -or $legacy.Count -ne 0 -or (@($camera.members.name) -join '|') -cne 'Configure|FirstVisibleCellX|FirstVisibleCellY|Follow|LastVisibleCellX|LastVisibleCellY|SmoothFollow' -or $apiText.Contains('::receiver')) { exit 1 } } finally { $zip.Dispose() }"
if errorlevel 1 exit /b 1
...
if errorlevel 1 exit /b 1
powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip=[IO.Compression.ZipFile]::OpenRead('%SMILE_ROOT%\artifacts\libraries\Smile.RPG.smilelib'); try { $manifest=([IO.StreamReader]::new($zip.GetEntry('manifest.json').Open())).ReadToEnd() | ConvertFrom-Json; $apiText=([IO.StreamReader]::new($zip.GetEntry('api/public-symbols.json').Open())).ReadToEnd(); $api=$apiText | ConvertFrom-Json; $names=@($api.modules.name); $members=@($api.modules.members); if ($manifest.formatVersion -ne 7 -or $api.formatVersion -ne 7 -or $manifest.provider -cne 'Smile.RPG@1.3.0' -or $api.library.provider -cne $manifest.provider -or $manifest.version -cne '1.3.0' -or @($manifest.dependencies).Count -ne 0 -or $names.Count -ne 15 -or $names -notcontains 'Smile.RPG.Core' -or $names -notcontains 'Smile.RPG.World' -or $names -notcontains 'Smile.RPG.Story' -or $names -notcontains 'Smile.RPG.Encounters' -or $names -notcontains 'Smile.RPG.BattleEffects' -or $names -notcontains 'Smile.RPG.BattleCore' -or $names -notcontains 'Smile.RPG.BattleStrategy' -or $names -notcontains 'Smile.RPG.BattleView' -or $names -notcontains 'Smile.RPG.SaveGames' -or @($api.modules | Where-Object { $_.provider -cne $manifest.provider }).Count -ne 0 -or @($members | Where-Object { $_.kind -in @('Enum','Class') }).Count -ne 0 -or @($members | Where-Object requiresGameWindow).Count -ne 0 -or $members.name -notcontains 'RPG_RESULT_NOT_SELLABLE' -or $members.name -notcontains 'RPG_RESULT_BATTLE_ACTIVE' -or $apiText.Contains('::receiver') -or $apiText.Contains('::value')) { exit 1 } } finally { $zip.Dispose() }"
if errorlevel 1 exit /b 1

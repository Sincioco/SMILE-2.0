"""Measure skinned body minimum Y at 30 Hz for the accepted Orin checkpoint.
...
raw=(root/'orin-v1.3-animation-checkpoint.glb').read_bytes()
...
meshes=[]
for n in doc['nodes']:
    if 'mesh' not in n or 'skin' not in n or n.get('name') in ['00_Shield','01_Weapon']: continue
...
result={}
for name in ['Idle','Defend','Hit','Death']:
    ...
    for time in np.linspace(0,duration,round(duration*30)+1):
...
report={'modelSha256':hashlib.sha256(raw).hexdigest(), 'sampleRate':30,
    ...
    'minimumY1000':result}
(root/'Calibration/orin-v1.3-grounding.json').write_text(json.dumps(report,indent=2)+'\n')

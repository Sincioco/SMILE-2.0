...
#define SMILE_3D_MAX_MESHES 256
...
#define SMILE_3D_MAX_GPU_PARTICLE_SYSTEMS 32
#define SMILE_3D_MAX_GPU_REFERENCE_PARTICLES 8192
#define SMILE_3D_MAX_GPU_PARTICLES_PER_SYSTEM 16384
...
#define SMILE_3D_GPU_PARTICLE_BACKEND_D3D11 1
...
#define SMILE_3D_GPU_PARTICLE_PIPELINE_NOT_ATTEMPTED 0
#define SMILE_3D_GPU_PARTICLE_PIPELINE_AVAILABLE 1
#define SMILE_3D_GPU_PARTICLE_PIPELINE_UNAVAILABLE 2
...
#define SMILE_3D_SOFT_DEPTH_OFF 0
#define SMILE_3D_SOFT_DEPTH_RGBA8_PACKED 1
#define SMILE_3D_SOFT_DEPTH_FLOAT16 2
#define SMILE_3D_SOFT_DEPTH_FLOAT32 3
#define SMILE_3D_SOFT_DEPTH_MATERIAL_OFF 0
#define SMILE_3D_SOFT_DEPTH_MATERIAL_AUTOMATIC 1
#define SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT 2
#define SMILE_3D_SOFT_DEPTH_FALLBACK_NONE 0
#define SMILE_3D_SOFT_DEPTH_FALLBACK_DISABLED 1
#define SMILE_3D_SOFT_DEPTH_FALLBACK_SHADER 2
#define SMILE_3D_SOFT_DEPTH_FALLBACK_TARGET 3
...
#define SMILE_3D_DISTORTION_FALLBACK_NONE 0
#define SMILE_3D_DISTORTION_FALLBACK_DISABLED 1
#define SMILE_3D_DISTORTION_FALLBACK_SHADER 2
#define SMILE_3D_DISTORTION_FALLBACK_TARGET 3
#define SMILE_3D_DISTORTION_FALLBACK_QUALITY 4
...
struct SmileMaterial3D
{
    ...
    unsigned char soft_depth_mode;
    float soft_depth_distance;
    unsigned char vfx_shading_mode;
    float distortion_strength;
    float distortion_noise_scale;
...
struct SmileGpuParticleConstants3D
{
    float step_seconds;
...
struct SmileGpuParticleSystem3D
{
    ...
    unsigned int* slot_ages;
    unsigned int* slot_lifetimes;
    unsigned char* slot_active;
    ID3D11Buffer* gpu_states[2];
...
static ID3D11ComputeShader* smile_gpu_particle_compute_shader3d;
static ID3D11VertexShader* smile_gpu_particle_vertex_shader3d;
static ID3D11InputLayout* smile_gpu_particle_input_layout3d;
static ID3D11Buffer* smile_particle_quad_vertex_buffer3d;
static ID3D11Buffer* smile_particle_quad_index_buffer3d;
static ID3D11VertexShader* smile_depth_copy_vertex_shader3d;
static ID3D11PixelShader* smile_depth_copy_pixel_shader3d;
static ID3D11PixelShader* smile_depth_copy_msaa_pixel_shader3d;
static ID3D11Buffer* smile_depth_copy_constant_buffer3d;
static ID3D11SamplerState* smile_depth_copy_sampler3d;
...
static ID3D11ShaderResourceView* smile_depth_shader_view3d;
static ID3D11Texture2D* smile_linear_depth_texture3d;
static ID3D11RenderTargetView* smile_linear_depth_view3d;
static ID3D11ShaderResourceView* smile_linear_depth_shader_view3d;
static ID3D11Texture2D* smile_distortion_texture3d;
static ID3D11RenderTargetView* smile_distortion_view3d;
static ID3D11ShaderResourceView* smile_distortion_shader_view3d;
static ID3D11Texture2D* smile_distortion_scratch_texture3d;
static ID3D11RenderTargetView* smile_distortion_scratch_view3d;
static ID3D11ShaderResourceView* smile_distortion_scratch_shader_view3d;
...
static int smile_reflection_pass3d;
...
static int smile_soft_depth_height3d;
static long long smile_soft_depth_bytes3d;
static long long smile_soft_depth_copy_draw_count3d;
static long long smile_soft_depth_copy_failure_count3d;
static long long smile_soft_particle_draw_count3d;
static int smile_soft_depth_resource_generation3d = 1;
static int smile_distortion_requested3d;
static int smile_distortion_quality3d = 3;
static int smile_distortion_effective3d;
static int smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_DISABLED;
static int smile_distortion_width3d;
static int smile_distortion_height3d;
static long long smile_distortion_bytes3d;
static long long smile_distortion_vector_draw_count3d;
static long long smile_distortion_composite_draw_count3d;
static long long smile_distortion_emitter_count3d;
...
static long long smile_gpu_particle_dispatch_count3d;
static long long smile_gpu_particle_render_draw_count3d;
static long long smile_gpu_particle_gpu_state_bytes3d;
static long long smile_gpu_particle_restart_count3d;
static int smile_gpu_particle_pipeline_state3d;
...
struct SmileM5TargetState3D
{
    ...
    ID3D11ShaderResourceView* depth_shader_view;
    ID3D11Texture2D* linear_depth_texture;
    ID3D11RenderTargetView* linear_depth_view;
    ID3D11ShaderResourceView* linear_depth_shader_view;
    ID3D11Texture2D* distortion_texture;
    ID3D11RenderTargetView* distortion_view;
    ID3D11ShaderResourceView* distortion_shader_view;
    ID3D11Texture2D* distortion_scratch_texture;
    ID3D11RenderTargetView* distortion_scratch_view;
    ID3D11ShaderResourceView* distortion_scratch_shader_view;
    ...
    int soft_depth_effective;
    int soft_depth_fallback_reason;
    int soft_depth_width;
    int soft_depth_height;
    long long soft_depth_bytes;
    int distortion_effective;
    int distortion_fallback_reason;
    int distortion_width;
    int distortion_height;
    long long distortion_bytes;
};
...
static int smile_3d_create_gpu_particle_pipeline(void);
static int smile_3d_ensure_gpu_particle_resources(SmileGpuParticleSystem3D* system);
...
static long long smile_3d_handle(long long kind, int slot, unsigned short generation)
{
    ...
    // Meshes use all 256 low-byte slots; generation remains sixteen bits.
    if (kind == SMILE_3D_MESH_HANDLE)
        return kind | ((long long)generation << 8) | (long long)slot;
...
static void smile_3d_release_gpu_particle_resources(SmileGpuParticleSystem3D* system)
{
    long long released_bytes = 0;
    if (system->gpu_states[0] != 0 || system->gpu_states[1] != 0)
        released_bytes = (long long)system->capacity * sizeof(SmileGpuParticleState3D) * 2 +
            sizeof(SmileGpuParticleConstants3D);
    smile_3d_release(system->gpu_constant_buffer);
    for (int index = 0; index < 2; ++index)
    {
        smile_3d_release(system->gpu_state_uavs[index]);
        smile_3d_release(system->gpu_state_views[index]);
        smile_3d_release(system->gpu_states[index]);
    }
    if (smile_gpu_particle_gpu_state_bytes3d >= released_bytes)
        smile_gpu_particle_gpu_state_bytes3d -= released_bytes;
    else smile_gpu_particle_gpu_state_bytes3d = 0;
}
...
static void smile_3d_delete_gpu_particle_system(SmileGpuParticleSystem3D* system)
{
    ...
    void* ages = system->slot_ages;
    void* lifetimes = system->slot_lifetimes;
    ...
    smile_3d_release_gpu_particle_resources(system);
    ...
    long long material_handle, int requested_simulation, int fixed_step_ms)
{
    ...
    unsigned int* ages;
    unsigned int* lifetimes;
    unsigned char* active;
    unsigned int maximum_capacity = requested_simulation == SMILE_3D_GPU_PARTICLE_SIMULATION_CPU
        ? SMILE_3D_MAX_GPU_REFERENCE_PARTICLES : SMILE_3D_MAX_GPU_PARTICLES_PER_SYSTEM;
    ...
    ages = (unsigned int*)smile_3d_allocate(sizeof(unsigned int) * capacity);
    lifetimes = (unsigned int*)smile_3d_allocate(sizeof(unsigned int) * capacity);
    active = (unsigned char*)smile_3d_allocate(sizeof(unsigned char) * capacity);
    if (state_a == 0 || state_b == 0 || staged == 0 || commands == 0 ||
        serials == 0 || ages == 0 || lifetimes == 0 || active == 0)
    {
        ...
        void* ages_allocation = ages;
        void* lifetimes_allocation = lifetimes;
        ...
        smile_3d_free(ages_allocation);
        smile_3d_free(lifetimes_allocation);
    ...
    ZeroMemory(ages, sizeof(unsigned int) * capacity);
    ZeroMemory(lifetimes, sizeof(unsigned int) * capacity);
    ZeroMemory(active, sizeof(unsigned char) * capacity);
    {
        ...
        system->slot_ages = ages;
        system->slot_lifetimes = lifetimes;
        system->slot_active = active;
        smile_gpu_particle_total_capacity3d += capacity;
        if (requested_simulation != SMILE_3D_GPU_PARTICLE_SIMULATION_CPU)
            smile_3d_ensure_gpu_particle_resources(system);
    ...
    long long slot, long long serial, long long seed)
{
    ...
    system->slot_ages[slot] = 0;
    system->slot_lifetimes[slot] =
        (unsigned int)llroundf(command->state.velocity_lifetime[3]);
...
static void smile_3d_step_gpu_particle_system(SmileGpuParticleSystem3D* system)
{
    ...
    for (unsigned int slot = 0; slot < system->capacity; ++slot)
    {
            ...
            destination[slot].velocity_lifetime[3])
        {
            ...
            system->slot_ages[slot] = system->slot_lifetimes[slot];
            destination[slot].seed_flags_gradient_frame[1] = 0;
...
static void smile_3d_apply_gpu_particle_spawns_fast(
    SmileGpuParticleSystem3D* system, ID3D11DeviceContext* context)
{
    ...
    for (unsigned int index = 0; index < system->pending_count; ++index)
    {
        ...
        if (command->slot >= system->capacity ||
            !system->slot_active[command->slot] ||
            system->slot_serials[command->slot] != command->serial) continue;
        box.left = command->slot * sizeof(SmileGpuParticleState3D);
        box.right = box.left + sizeof(SmileGpuParticleState3D);
        box.bottom = 1;
        box.back = 1;
        context->UpdateSubresource(system->gpu_states[system->read_index], 0,
            &box, &command->state, 0, 0);
    }
    if (system->pending_count != 0)
    {
        long long bytes = (long long)system->pending_count *
...
static int smile_3d_step_gpu_particle_system_fast(SmileGpuParticleSystem3D* system)
{
    ID3D11DeviceContext* context =
        (ID3D11DeviceContext*)smile_graphics_directx_context();
    ID3D11ShaderResourceView* no_view = 0;
    ID3D11UnorderedAccessView* no_uav = 0;
    ID3D11Buffer* no_buffer = 0;
    UINT initial_count = 0;
    SmileGpuParticleConstants3D constants = {};
    if (context == 0 || !smile_3d_ensure_gpu_particle_resources(system)) return 0;
    smile_3d_apply_gpu_particle_spawns_fast(system, context);
    ...
    context->CSSetUnorderedAccessViews(0, 1,
        &system->gpu_state_uavs[1 - system->read_index], &initial_count);
    context->CSSetConstantBuffers(0, 1, &system->gpu_constant_buffer);
    context->CSSetShader(smile_gpu_particle_compute_shader3d, 0, 0);
    context->Dispatch((system->capacity + 255) / 256, 1, 1);
    context->CSSetUnorderedAccessViews(0, 1, &no_uav, &initial_count);
    context->CSSetShaderResources(0, 1, &no_view);
    context->CSSetConstantBuffers(0, 1, &no_buffer);
    context->CSSetShader(0, 0, 0);
    for (unsigned int slot = 0; slot < system->capacity; ++slot)
    {
        if (!system->slot_active[slot]) continue;
        system->slot_ages[slot] += system->fixed_step_ms;
        if (system->slot_ages[slot] < system->slot_lifetimes[slot]) continue;
        system->slot_ages[slot] = system->slot_lifetimes[slot];
        system->slot_active[slot] = 0;
        if (system->active_count != 0) system->active_count--;
    }
    ...
    system->simulation_steps++;
    smile_gpu_particle_simulation_steps3d++;
    smile_gpu_particle_dispatch_count3d++;
    return 1;
}
    ...
    long long elapsed_ms)
{
    ...
    {
        if (system->effective_simulation == SMILE_3D_GPU_PARTICLE_SIMULATION_FAST &&
            !smile_3d_step_gpu_particle_system_fast(system))
            system->effective_simulation = SMILE_3D_GPU_PARTICLE_SIMULATION_CPU;
    ...
    long long slot)
{
    ...
    if (context != 0 && system->effective_simulation ==
        SMILE_3D_GPU_PARTICLE_SIMULATION_FAST && system->gpu_states[0] != 0)
    {
        SmileGpuParticleState3D inactive = {};
        ID3D11ShaderResourceView* no_view = 0;
        D3D11_BOX box = {};
        box.left = (UINT)slot * sizeof(SmileGpuParticleState3D);
        box.right = box.left + sizeof(SmileGpuParticleState3D);
        box.bottom = 1;
        box.back = 1;
        context->VSSetShaderResources(7, 1, &no_view);
        for (int index = 0; index < 2; ++index)
            context->UpdateSubresource(system->gpu_states[index], 0,
                &box, &inactive, 0, 0);
        system->upload_bytes += sizeof(SmileGpuParticleState3D) * 2;
        smile_gpu_particle_cpu_upload_bytes3d += sizeof(SmileGpuParticleState3D) * 2;
    }
    return 1;
}
...
static long long smile_3d_soft_depth_command(long long operation,
    long long b, long long c, long long d)
{
    if (operation == 1)
    {
        if (smile_frame_active3d || (b != 0 && b != 1))
        {
            smile_last_error3d = 65;
            return 0;
        }
        if (smile_soft_depth_requested3d != (int)b)
        {
            smile_soft_depth_requested3d = (int)b;
    ...
    if (operation == 2)
    {
        ...
        if (material == 0 || material->mode != 0 ||
            (material->alpha_mode != 2 && material->alpha_mode != 3) ||
            c < SMILE_3D_SOFT_DEPTH_MATERIAL_OFF ||
            c > SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT ||
            (c == SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT && (d <= 0 || d > 1000000)) ||
            (c != SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT && d != 0))
        {
            smile_last_error3d = 65;
            return 0;
        }
        material->soft_depth_mode = (unsigned char)c;
        material->soft_depth_distance = c == SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT
            ? (float)d : 0.0f;
        return 1;
    }
    if (operation == 3)
    {
        if (b == 1) return smile_soft_depth_requested3d;
        if (b == 2 || b == 5) return smile_soft_depth_effective3d;
        if (b == 3) return smile_soft_depth_width3d;
        if (b == 4) return smile_soft_depth_height3d;
        if (b == 6) return smile_soft_depth_bytes3d;
        if (b == 7) return smile_soft_depth_copy_draw_count3d;
        if (b == 8) return smile_soft_depth_copy_failure_count3d;
        if (b == 9) return smile_soft_particle_draw_count3d;
        if (b == 10)
        {
            long long count = 0;
            for (int index = 0; index < SMILE_3D_MAX_MATERIALS; ++index)
                if (smile_materials3d[index].active &&
                    smile_materials3d[index].soft_depth_mode !=
                        SMILE_3D_SOFT_DEPTH_MATERIAL_OFF)
                    count++;
            return count;
        }
        if (b == 11) return smile_soft_depth_fallback_reason3d;
        if (b == 12) return smile_soft_depth_resource_generation3d;
        if (b == 13 || b == 14)
        {
            SmileMaterial3D* material = smile_3d_material(c);
            if (material == 0)
            {
                smile_last_error3d = 65;
                return 0;
            }
            return b == 13 ? material->soft_depth_mode :
                (long long)llroundf(material->soft_depth_distance);
        }
    }
    smile_last_error3d = 65;
    return 0;
...
static long long smile_3d_distortion_command(long long operation,
    ...
    long long g)
{
    ...
    if (operation == 1)
    {
        ...
        {
            smile_last_error3d = SMILE_3D_DISTORTION_ERROR_INVALID;
            return 0;
        }
        if (smile_distortion_requested3d != (int)b ||
            smile_distortion_quality3d != (int)(c == 0 ? 3 : c))
        {
            smile_distortion_requested3d = (int)b;
            smile_distortion_quality3d = (int)(c == 0 ? 3 : c);
            smile_m5_configuration_revision3d++;
            if (smile_m5_configuration_revision3d <= 0 ||
                smile_m5_configuration_revision3d > 2147483647)
    ...
    if (operation == 2)
    {
        ...
        if (material == 0 || material->mode != 0 ||
            (material->alpha_mode != 2 && material->alpha_mode != 3) ||
            c < 0 || c > 100 || d < 1 || d > 1000 || e < -1000 || e > 1000 ||
            f < -100 || f > 100 || g < -100 || g > 100 ||
            (c > 0 && f == 0 && g == 0))
        {
            smile_last_error3d = SMILE_3D_DISTORTION_ERROR_INVALID;
            return 0;
        }
        material->vfx_shading_mode = c == 0
            ? SMILE_3D_VFX_SHADING_STANDARD : SMILE_3D_VFX_SHADING_DISTORTION;
        material->distortion_strength = (float)c;
        material->distortion_noise_scale = (float)d;
        material->distortion_noise_speed = (float)e;
        material->distortion_flow_x = (float)f;
        material->distortion_flow_y = (float)g;
        return 1;
    }
    if (operation == 3)
    {
        if (b == 1) return smile_distortion_requested3d;
        if (b == 2 || b == 5) return smile_distortion_effective3d;
        if (b == 3) return smile_distortion_width3d;
        if (b == 4) return smile_distortion_height3d;
        if (b == 6) return smile_distortion_bytes3d;
        if (b == 7) return smile_distortion_emitter_count3d;
        if (b == 8) return smile_distortion_vector_draw_count3d;
        if (b == 9) return smile_distortion_composite_draw_count3d;
        if (b == 10) return smile_distortion_maximum_strength3d;
        if (b == 11) return smile_distortion_fallback_reason3d;
        if (b == 12) return smile_distortion_resource_generation3d;
        if (b == 15) return smile_distortion_quality3d;
        if (b == 13 || b == 14)
        {
            SmileMaterial3D* material = smile_3d_material(c);
            if (material == 0)
            ...
            return b == 13 ? material->vfx_shading_mode :
                (long long)llroundf(material->distortion_strength);
        }
    }
    smile_last_error3d = SMILE_3D_DISTORTION_ERROR_INVALID;
    return 0;
...
static void smile_3d_active_camera_position(float output[3])
{
    memcpy(output, smile_camera_position3d, sizeof(smile_camera_position3d));
    if (smile_reflection_pass3d)
        output[1] = 2.0f * smile_reflections_floor_height() - output[1];
}

static void smile_3d_set_reflection_constants(const SmileObject3D* object,
    float reflection[4], float viewport[4])
{
    reflection[0] = smile_reflection_pass3d ? 1.0f : 0.0f;
    reflection[1] = smile_reflections_floor_height();
    reflection[2] = !smile_reflection_pass3d && object->reflection_mode == 2 &&
        smile_reflections_effective()
...
static void smile_3d_set_object_raster(ID3D11DeviceContext* context,
    const SmileObject3D* object, int double_sided)
{
    int cull_mode = object->cull_mode;
    if (cull_mode == 1 || (cull_mode == 0 && double_sided))
    {
        context->RSSetState(smile_raster_state3d);
        return;
    }
    if (cull_mode == 0 || cull_mode == 2)
        context->RSSetState(smile_reflection_pass3d
            ? smile_front_cull_raster_state3d : smile_cull_raster_state3d);
    else
        context->RSSetState(smile_reflection_pass3d
            ? smile_cull_raster_state3d : smile_front_cull_raster_state3d);
}
...
static int smile_3d_create_gpu_particle_pipeline(void)
{
    ...
    ID3D11ComputeShader* compute_shader = 0;
    ID3D11VertexShader* vertex_shader = 0;
    ID3D11InputLayout* input_layout = 0;
    D3D11_INPUT_ELEMENT_DESC elements[2] = {};
    WCHAR forced[2];
    HRESULT result;
    if (smile_gpu_particle_pipeline_state3d ==
        SMILE_3D_GPU_PARTICLE_PIPELINE_AVAILABLE) return 1;
    if (smile_gpu_particle_pipeline_state3d ==
        SMILE_3D_GPU_PARTICLE_PIPELINE_UNAVAILABLE) return 0;
    if (device == 0 || device->GetFeatureLevel() < D3D_FEATURE_LEVEL_11_0 ||
        GetEnvironmentVariableW(L"SMILE_TEST_RENDERER3D_FORCE_GPU_PARTICLE_SHADER_FAILURE",
            forced, 2) != 0 || !smile_3d_create_pipeline())
    {
        smile_gpu_particle_pipeline_state3d =
            SMILE_3D_GPU_PARTICLE_PIPELINE_UNAVAILABLE;
        return 0;
    }
    result = smile_3d_compile(device, compute_source, "main", "cs_5_0", &compute_bytecode);
    if (SUCCEEDED(result))
        result = smile_3d_compile(device, vertex_source, "main", "vs_5_0", &vertex_bytecode);
    if (SUCCEEDED(result)) result = device->CreateComputeShader(
        compute_bytecode->GetBufferPointer(), compute_bytecode->GetBufferSize(), 0,
        &compute_shader);
    if (SUCCEEDED(result)) result = device->CreateVertexShader(
        vertex_bytecode->GetBufferPointer(), vertex_bytecode->GetBufferSize(), 0,
        &vertex_shader);
    ...
    if (SUCCEEDED(result)) result = device->CreateInputLayout(
        elements, 2, vertex_bytecode->GetBufferPointer(), vertex_bytecode->GetBufferSize(),
        &input_layout);
    smile_3d_release(compute_bytecode);
    smile_3d_release(vertex_bytecode);
    if (FAILED(result))
    {
        smile_3d_release(input_layout);
        smile_3d_release(vertex_shader);
        smile_3d_release(compute_shader);
        smile_gpu_particle_pipeline_state3d =
            SMILE_3D_GPU_PARTICLE_PIPELINE_UNAVAILABLE;
        return 0;
    }
    smile_gpu_particle_compute_shader3d = compute_shader;
    smile_gpu_particle_vertex_shader3d = vertex_shader;
    smile_gpu_particle_input_layout3d = input_layout;
    smile_gpu_particle_pipeline_state3d = SMILE_3D_GPU_PARTICLE_PIPELINE_AVAILABLE;
    return 1;
}
...
static int smile_3d_ensure_gpu_particle_resources(SmileGpuParticleSystem3D* system)
{
    ...
    WCHAR forced[2];
    HRESULT result = S_OK;
    if (system->requested_simulation == SMILE_3D_GPU_PARTICLE_SIMULATION_CPU) return 0;
    if (system->gpu_states[0] != 0 && system->gpu_states[1] != 0)
    {
        system->effective_simulation = SMILE_3D_GPU_PARTICLE_SIMULATION_FAST;
        return 1;
    }
    if (device == 0 || !smile_3d_create_gpu_particle_pipeline() ||
        GetEnvironmentVariableW(L"SMILE_TEST_RENDERER3D_FORCE_GPU_PARTICLE_BUFFER_FAILURE",
            forced, 2) != 0)
    {
        system->effective_simulation = SMILE_3D_GPU_PARTICLE_SIMULATION_CPU;
        return 0;
    }
    description.ByteWidth = sizeof(SmileGpuParticleState3D) * system->capacity;
    ...
    description.StructureByteStride = sizeof(SmileGpuParticleState3D);
    initial.pSysMem = system->states[system->read_index];
    result = device->CreateBuffer(&description, &initial, &states[0]);
    if (SUCCEEDED(result)) result = device->CreateBuffer(&description, &initial, &states[1]);
    view_description.Format = DXGI_FORMAT_UNKNOWN;
    view_description.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
    view_description.Buffer.NumElements = system->capacity;
    uav_description.Format = DXGI_FORMAT_UNKNOWN;
    uav_description.ViewDimension = D3D11_UAV_DIMENSION_BUFFER;
    uav_description.Buffer.NumElements = system->capacity;
    for (int index = 0; index < 2 && SUCCEEDED(result); ++index)
    {
        result = device->CreateShaderResourceView(
            states[index], &view_description, &views[index]);
        if (SUCCEEDED(result)) result = device->CreateUnorderedAccessView(
            states[index], &uav_description, &uavs[index]);
    }
    description = {};
    description.ByteWidth = sizeof(SmileGpuParticleConstants3D);
    description.Usage = D3D11_USAGE_DEFAULT;
    description.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    if (SUCCEEDED(result)) result = device->CreateBuffer(
        &description, 0, &constant_buffer);
    if (FAILED(result))
    {
        smile_3d_release(constant_buffer);
        for (int index = 0; index < 2; ++index)
        {
            smile_3d_release(uavs[index]);
            smile_3d_release(views[index]);
            smile_3d_release(states[index]);
        }
        system->effective_simulation = SMILE_3D_GPU_PARTICLE_SIMULATION_CPU;
        return 0;
    }
    for (int index = 0; index < 2; ++index)
    {
        system->gpu_states[index] = states[index];
        system->gpu_state_views[index] = views[index];
        system->gpu_state_uavs[index] = uavs[index];
    }
    system->gpu_constant_buffer = constant_buffer;
    system->effective_simulation = SMILE_3D_GPU_PARTICLE_SIMULATION_FAST;
    smile_gpu_particle_gpu_state_bytes3d +=
        (long long)system->capacity * sizeof(SmileGpuParticleState3D) * 2 +
        sizeof(SmileGpuParticleConstants3D);
    return 1;
}
...
static int smile_3d_create_pipeline(void)
{
    ...
    if (smile_soft_depth_requested3d && smile_depth_copy_vertex_shader3d == 0)
    {
        ...
        result = smile_3d_compile(
            device, depth_copy_vertex_source, "main", "vs_4_0", &depth_vs);
        if (SUCCEEDED(result)) result = smile_3d_compile(
            device, depth_copy_pixel_source, "main", "ps_4_0", &depth_ps);
        if (SUCCEEDED(result)) result = smile_3d_compile(
            device, depth_copy_msaa_pixel_source, "main", "ps_5_0", &depth_msaa_ps);
        if (SUCCEEDED(result)) result = device->CreateVertexShader(
            depth_vs->GetBufferPointer(), depth_vs->GetBufferSize(), 0,
            &smile_depth_copy_vertex_shader3d);
        if (SUCCEEDED(result)) result = device->CreatePixelShader(
            depth_ps->GetBufferPointer(), depth_ps->GetBufferSize(), 0,
            &smile_depth_copy_pixel_shader3d);
        if (SUCCEEDED(result)) result = device->CreatePixelShader(
            depth_msaa_ps->GetBufferPointer(), depth_msaa_ps->GetBufferSize(), 0,
            &smile_depth_copy_msaa_pixel_shader3d);
        depth_buffer.ByteWidth = sizeof(SmileDepthConstants3D);
        depth_buffer.Usage = D3D11_USAGE_DEFAULT;
        depth_buffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        if (SUCCEEDED(result)) result = device->CreateBuffer(
            &depth_buffer, 0, &smile_depth_copy_constant_buffer3d);
        depth_sampler.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
        depth_sampler.AddressU = depth_sampler.AddressV = depth_sampler.AddressW =
            D3D11_TEXTURE_ADDRESS_CLAMP;
        depth_sampler.MaxLOD = D3D11_FLOAT32_MAX;
        if (SUCCEEDED(result)) result = device->CreateSamplerState(
            &depth_sampler, &smile_depth_copy_sampler3d);
        smile_3d_release(depth_vs);
        smile_3d_release(depth_ps);
        smile_3d_release(depth_msaa_ps);
        if (FAILED(result))
        {
            smile_3d_release(smile_depth_copy_sampler3d);
            smile_3d_release(smile_depth_copy_constant_buffer3d);
            smile_3d_release(smile_depth_copy_msaa_pixel_shader3d);
            smile_3d_release(smile_depth_copy_pixel_shader3d);
            smile_3d_release(smile_depth_copy_vertex_shader3d);
            smile_soft_depth_effective3d = SMILE_3D_SOFT_DEPTH_OFF;
            smile_soft_depth_fallback_reason3d = SMILE_3D_SOFT_DEPTH_FALLBACK_SHADER;
        }
    }
...
static int smile_3d_create_depth_view(ID3D11Device* device, ID3D11Texture2D* texture,
    UINT samples, ID3D11DepthStencilView** view)
{
    D3D11_DEPTH_STENCIL_VIEW_DESC description = {};
    description.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
    description.ViewDimension = samples > 1
        ? D3D11_DSV_DIMENSION_TEXTURE2DMS : D3D11_DSV_DIMENSION_TEXTURE2D;
    return SUCCEEDED(device->CreateDepthStencilView(texture, &description, view));
}

static int smile_3d_create_soft_depth_targets(ID3D11Device* device, int width, int height,
    UINT samples)
{
    ...
    if (smile_depth_copy_vertex_shader3d == 0 || smile_depth_copy_pixel_shader3d == 0 ||
        smile_depth_copy_msaa_pixel_shader3d == 0)
    {
        smile_soft_depth_copy_failure_count3d++;
        smile_soft_depth_effective3d = SMILE_3D_SOFT_DEPTH_OFF;
        smile_soft_depth_fallback_reason3d = SMILE_3D_SOFT_DEPTH_FALLBACK_SHADER;
        return 1;
    }
    if (GetEnvironmentVariableW(L"SMILE_TEST_RENDERER3D_FORCE_SOFT_DEPTH_FAILURE",
        forced_failure, 2) != 0)
    {
        smile_soft_depth_copy_failure_count3d++;
        smile_soft_depth_effective3d = SMILE_3D_SOFT_DEPTH_OFF;
        smile_soft_depth_fallback_reason3d = SMILE_3D_SOFT_DEPTH_FALLBACK_TARGET;
        return 1;
    }
    source_view.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
    source_view.ViewDimension = samples > 1
        ? D3D11_SRV_DIMENSION_TEXTURE2DMS : D3D11_SRV_DIMENSION_TEXTURE2D;
    source_view.Texture2D.MipLevels = 1;
    result = device->CreateShaderResourceView(
        smile_depth_texture3d, &source_view, &smile_depth_shader_view3d);
    linear.Width = (UINT)width;
    linear.Height = (UINT)height;
    linear.MipLevels = linear.ArraySize = 1;
    linear.Format = DXGI_FORMAT_R32_FLOAT;
    linear.SampleDesc.Count = 1;
    linear.Usage = D3D11_USAGE_DEFAULT;
    linear.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
    if (SUCCEEDED(result)) result = device->CreateTexture2D(
        &linear, 0, &smile_linear_depth_texture3d);
    if (SUCCEEDED(result)) result = device->CreateRenderTargetView(
        smile_linear_depth_texture3d, 0, &smile_linear_depth_view3d);
    if (SUCCEEDED(result)) result = device->CreateShaderResourceView(
        smile_linear_depth_texture3d, 0, &smile_linear_depth_shader_view3d);
    if (FAILED(result))
    {
        smile_soft_depth_copy_failure_count3d++;
    ...
    smile_soft_depth_effective3d = SMILE_3D_SOFT_DEPTH_FLOAT32;
    smile_soft_depth_fallback_reason3d = SMILE_3D_SOFT_DEPTH_FALLBACK_NONE;
    smile_soft_depth_width3d = width;
    smile_soft_depth_height3d = height;
    smile_soft_depth_bytes3d = (long long)width * height * 4;
    return 1;
}

static int smile_3d_create_sampleable_color_target(ID3D11Device* device,
    int width, int height, DXGI_FORMAT format, ID3D11Texture2D** texture,
    ID3D11RenderTargetView** target, ID3D11ShaderResourceView** view)
{
    D3D11_TEXTURE2D_DESC description = {};
    HRESULT result;
    description.Width = (UINT)width;
    ...
    if (FAILED(result))
    {
        smile_3d_release(*view);
        smile_3d_release(*target);
        smile_3d_release(*texture);
        return 0;
    }
    return 1;
}

static int smile_3d_create_distortion_targets(ID3D11Device* device, int width, int height)
{
    WCHAR forced_failure[2];
    int divisor;
    DXGI_FORMAT scene_format;
    if (!smile_distortion_requested3d)
    {
        smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
        smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_DISABLED;
        return 1;
    }
    if (smile_distortion_quality3d == 1)
    {
        smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
        smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_QUALITY;
        return 1;
    }
    if (smile_post_vertex_shader3d == 0 || smile_post_pixel_shader3d == 0 ||
        smile_scene_shader_view3d == 0 || smile_color_texture3d == 0 ||
        GetEnvironmentVariableW(L"SMILE_TEST_RENDERER3D_FORCE_DISTORTION_FAILURE",
            forced_failure, 2) != 0)
    {
        smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
        smile_distortion_fallback_reason3d = smile_post_pixel_shader3d == 0
            ? SMILE_3D_DISTORTION_FALLBACK_SHADER : SMILE_3D_DISTORTION_FALLBACK_TARGET;
        return 1;
    }
    divisor = smile_distortion_quality3d == 2 ? 4 : 2;
    smile_distortion_width3d = width / divisor;
    smile_distortion_height3d = height / divisor;
    if (smile_distortion_width3d < 1) smile_distortion_width3d = 1;
    if (smile_distortion_height3d < 1) smile_distortion_height3d = 1;
    scene_format = smile_hdr_effective3d
        ? DXGI_FORMAT_R16G16B16A16_FLOAT : DXGI_FORMAT_B8G8R8A8_UNORM;
    if (!smile_3d_create_sampleable_color_target(device,
            smile_distortion_width3d, smile_distortion_height3d,
            DXGI_FORMAT_R16G16B16A16_FLOAT, &smile_distortion_texture3d,
            &smile_distortion_view3d, &smile_distortion_shader_view3d) ||
        !smile_3d_create_sampleable_color_target(device, width, height, scene_format,
            &smile_distortion_scratch_texture3d, &smile_distortion_scratch_view3d,
            &smile_distortion_scratch_shader_view3d))
    {
        smile_3d_release(smile_distortion_shader_view3d);
        smile_3d_release(smile_distortion_view3d);
        smile_3d_release(smile_distortion_texture3d);
        smile_3d_release(smile_distortion_scratch_shader_view3d);
        smile_3d_release(smile_distortion_scratch_view3d);
        smile_3d_release(smile_distortion_scratch_texture3d);
        smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
        smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_TARGET;
        smile_distortion_width3d = smile_distortion_height3d = 0;
        smile_distortion_bytes3d = 0;
        return 1;
    }
    smile_distortion_effective3d = SMILE_3D_DISTORTION_RGBA16_FLOAT;
    smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_NONE;
    smile_distortion_bytes3d =
        (long long)smile_distortion_width3d * smile_distortion_height3d * 8 +
        (long long)width * height * (smile_hdr_effective3d ? 8 : 4);
    return 1;
}
...
static SmileM5TargetState3D smile_3d_take_target_state(void)
{
    ...
    state.depth_shader_view = smile_depth_shader_view3d;
    state.linear_depth_texture = smile_linear_depth_texture3d;
    state.linear_depth_view = smile_linear_depth_view3d;
    state.linear_depth_shader_view = smile_linear_depth_shader_view3d;
    state.distortion_texture = smile_distortion_texture3d;
    state.distortion_view = smile_distortion_view3d;
    state.distortion_shader_view = smile_distortion_shader_view3d;
    state.distortion_scratch_texture = smile_distortion_scratch_texture3d;
    state.distortion_scratch_view = smile_distortion_scratch_view3d;
    state.distortion_scratch_shader_view = smile_distortion_scratch_shader_view3d;
    ...
    state.soft_depth_effective = smile_soft_depth_effective3d;
    state.soft_depth_fallback_reason = smile_soft_depth_fallback_reason3d;
    state.soft_depth_width = smile_soft_depth_width3d;
    state.soft_depth_height = smile_soft_depth_height3d;
    state.soft_depth_bytes = smile_soft_depth_bytes3d;
    state.distortion_effective = smile_distortion_effective3d;
    state.distortion_fallback_reason = smile_distortion_fallback_reason3d;
    state.distortion_width = smile_distortion_width3d;
    state.distortion_height = smile_distortion_height3d;
    state.distortion_bytes = smile_distortion_bytes3d;
    return state;
}
...
static void smile_3d_clear_target_state(void)
{
    ...
    smile_depth_shader_view3d = 0;
    smile_linear_depth_texture3d = 0; smile_linear_depth_view3d = 0;
    smile_linear_depth_shader_view3d = 0;
    smile_distortion_texture3d = 0; smile_distortion_view3d = 0;
    smile_distortion_shader_view3d = 0;
    smile_distortion_scratch_texture3d = 0; smile_distortion_scratch_view3d = 0;
    smile_distortion_scratch_shader_view3d = 0;
    ...
    smile_soft_depth_bytes3d = 0;
    smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
    smile_distortion_fallback_reason3d = smile_distortion_requested3d
        ? SMILE_3D_DISTORTION_FALLBACK_TARGET
        : SMILE_3D_DISTORTION_FALLBACK_DISABLED;
    smile_distortion_width3d = smile_distortion_height3d = 0;
    smile_distortion_bytes3d = 0;
}
...
static void smile_3d_restore_target_state(const SmileM5TargetState3D& state)
{
    ...
    smile_depth_shader_view3d = state.depth_shader_view;
    smile_linear_depth_texture3d = state.linear_depth_texture;
    smile_linear_depth_view3d = state.linear_depth_view;
    smile_linear_depth_shader_view3d = state.linear_depth_shader_view;
    smile_distortion_texture3d = state.distortion_texture;
    smile_distortion_view3d = state.distortion_view;
    smile_distortion_shader_view3d = state.distortion_shader_view;
    smile_distortion_scratch_texture3d = state.distortion_scratch_texture;
    smile_distortion_scratch_view3d = state.distortion_scratch_view;
    smile_distortion_scratch_shader_view3d = state.distortion_scratch_shader_view;
    ...
    smile_soft_depth_effective3d = state.soft_depth_effective;
    smile_soft_depth_fallback_reason3d = state.soft_depth_fallback_reason;
    smile_soft_depth_width3d = state.soft_depth_width;
    smile_soft_depth_height3d = state.soft_depth_height;
    smile_soft_depth_bytes3d = state.soft_depth_bytes;
    smile_distortion_effective3d = state.distortion_effective;
    smile_distortion_fallback_reason3d = state.distortion_fallback_reason;
    smile_distortion_width3d = state.distortion_width;
    smile_distortion_height3d = state.distortion_height;
    smile_distortion_bytes3d = state.distortion_bytes;
}
...
static void smile_3d_release_target_state(SmileM5TargetState3D& state)
{
    ...
    smile_3d_release(state.depth_shader_view);
    smile_3d_release(state.linear_depth_shader_view); smile_3d_release(state.linear_depth_view);
    smile_3d_release(state.linear_depth_texture);
    smile_3d_release(state.distortion_shader_view); smile_3d_release(state.distortion_view);
    smile_3d_release(state.distortion_texture);
    smile_3d_release(state.distortion_scratch_shader_view);
    smile_3d_release(state.distortion_scratch_view);
    smile_3d_release(state.distortion_scratch_texture);
}
...
static int smile_3d_create_targets(void)
{
    ...
    smile_3d_create_soft_depth_targets(device, width, height, smile_sample_count3d);
    smile_3d_create_distortion_targets(device, width, height);
    ...
    smile_bloom_bytes3d = smile_bloom_effective3d
        ...
        : 0;
    smile_m5_target_bytes3d = smile_shadow_bytes3d + smile_scene_bytes3d +
        smile_bloom_bytes3d + smile_soft_depth_bytes3d + smile_distortion_bytes3d;
    smile_m5_applied_revision3d = smile_m5_configuration_revision3d;
    ...
    if (smile_soft_depth_effective3d != SMILE_3D_SOFT_DEPTH_OFF)
    {
        smile_soft_depth_resource_generation3d++;
        if (smile_soft_depth_resource_generation3d <= 0 ||
            smile_soft_depth_resource_generation3d > 2147483647)
            smile_soft_depth_resource_generation3d = 1;
    }
    if (smile_distortion_effective3d != SMILE_3D_DISTORTION_OFF)
    {
        smile_distortion_resource_generation3d++;
        if (smile_distortion_resource_generation3d <= 0 ||
            smile_distortion_resource_generation3d > 2147483647)
            smile_distortion_resource_generation3d = 1;
    }
    smile_3d_release_target_state(previous);
    return 1;
}
...
static int smile_3d_prepare_m5_resources(void)
{
    ...
    smile_multipass_active3d = smile_shadow_effective3d || smile_hdr_effective3d ||
        smile_soft_depth_effective3d != SMILE_3D_SOFT_DEPTH_OFF ||
        smile_distortion_requested3d || smile_reflections_requested();
    smile_m5_target_bytes3d = smile_shadow_bytes3d + smile_scene_bytes3d +
        smile_bloom_bytes3d + smile_soft_depth_bytes3d + smile_distortion_bytes3d;
    return 1;
}
...
static int smile_3d_begin(long long red, long long green, long long blue)
{
    ...
    smile_soft_depth_copy_draw_count3d = 0;
    smile_soft_particle_draw_count3d = 0;
...
static int smile_3d_draw_pbr(const SmileSubmission3D* submission)
{
    ...
    smile_3d_active_camera_position(active_camera);
    memcpy(constants.camera_position, active_camera, sizeof(active_camera));
    ...
    ID3D11ShaderResourceView* reflection_view =
        object->reflection_mode == 2 ? smile_reflections_shader_view() : 0;
    ID3D11SamplerState* reflection_sampler =
        object->reflection_mode == 2 ? smile_reflections_sampler() : 0;
    context->PSSetShaderResources(4, 1, &reflection_view);
    ...
    if (smile_reflection_pass3d)
        smile_reflections_complete_capture(1, mesh->index_count / 3);
    else
    {
        ...
        if (object->reflection_mode == 2 && smile_reflections_effective())
...
static int smile_3d_draw_vfx_submission(const SmileSubmission3D* submission)
{
    ...
    ID3D11ShaderResourceView* soft_depth_view = 0;
    ID3D11SamplerState* soft_depth_sampler = 0;
    SmileVfxConstants3D constants = {};
    SmileMatrix3D view;
    SmileMatrix3D projection;
    float aspect;
    int soft_depth_enabled;
    ...
    constants.soft_depth[1] = material->soft_depth_mode ==
        SMILE_3D_SOFT_DEPTH_MATERIAL_EXPLICIT
        ? material->soft_depth_distance : SMILE_3D_SOFT_DEPTH_AUTOMATIC_DISTANCE;
    constants.soft_depth[2] = smile_camera_near3d;
    ...
    if (smile_rendering_distortion_vectors3d)
    {
        ...
        constants.target[1] = (float)smile_distortion_height3d;
        constants.target[2] = material->distortion_flow_x / 100.0f;
        constants.target[3] = material->distortion_flow_y / 100.0f;
        constants.distortion[0] = 1.0f;
        constants.distortion[1] = material->distortion_strength / 10000.0f;
        constants.distortion[2] = material->distortion_noise_scale / 100.0f;
        constants.distortion[3] = material->distortion_noise_speed / 1000.0f;
    }
    if (soft_depth_enabled)
    {
        soft_depth_view = smile_linear_depth_shader_view3d;
        soft_depth_sampler = smile_depth_copy_sampler3d;
    }
    ...
    context->PSSetShaderResources(6, 1, &soft_depth_view);
...
static int smile_3d_draw_gpu_particle_system(SmileGpuParticleSystem3D* system)
{
    ...
    if (system->effective_simulation != SMILE_3D_GPU_PARTICLE_SIMULATION_FAST)
        return 1;
    if (context == 0 || material == 0)
    { smile_last_error3d = SMILE_3D_GPU_PARTICLE_ERROR_INVALID; return 0; }
    if (!smile_3d_ensure_gpu_particle_resources(system))
        return system->effective_simulation == SMILE_3D_GPU_PARTICLE_SIMULATION_CPU;
    if (smile_gpu_particle_vertex_shader3d == 0 || smile_vfx_pixel_shader3d == 0)
    { smile_last_error3d = SMILE_3D_GPU_PARTICLE_ERROR_INVALID; return 0; }
    if (material->texture_handles[0] != 0)
    {
        texture = smile_3d_texture(material->texture_handles[0]);
    ...
    context->OMSetBlendState(smile_rendering_distortion_vectors3d ||
        material->alpha_mode == 3 ? smile_additive_blend_state3d : smile_blend_state3d,
        0, 0xffffffff);
    context->OMSetDepthStencilState(smile_depth_read_state3d, 0);
    context->RSSetState(smile_raster_state3d);
    context->UpdateSubresource(smile_vfx_constant_buffer3d, 0, 0, &constants, 0, 0);
    context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    context->IASetInputLayout(smile_gpu_particle_input_layout3d);
    context->IASetVertexBuffers(0, 1, &smile_particle_quad_vertex_buffer3d,
        &stride, &offset);
    context->IASetIndexBuffer(smile_particle_quad_index_buffer3d,
        DXGI_FORMAT_R16_UINT, 0);
    context->VSSetShader(smile_gpu_particle_vertex_shader3d, 0, 0);
    context->VSSetConstantBuffers(0, 1, &smile_vfx_constant_buffer3d);
    context->VSSetShaderResources(7, 1, &system->gpu_state_views[system->read_index]);
    context->PSSetShader(smile_vfx_pixel_shader3d, 0, 0);
    context->PSSetConstantBuffers(0, 1, &smile_vfx_constant_buffer3d);
    context->PSSetShaderResources(0, 1, &texture_view);
    context->PSSetSamplers(0, 1, &texture_sampler);
...
static int smile_3d_render_reflection_pass(void)
{
    ID3D11Device* device;
    ID3D11DeviceContext* context;
    ID3D11RenderTargetView* target;
    ...
    context->OMSetRenderTargets(1, &target, smile_reflections_depth());
    context->OMSetDepthStencilState(smile_depth_state3d, 0);
    context->OMSetBlendState(0, 0, 0xffffffff);
    viewport.Width = (FLOAT)smile_reflections_width();
    viewport.Height = (FLOAT)smile_reflections_height();
    viewport.MinDepth = 0.0f;
    viewport.MaxDepth = 1.0f;
    context->RSSetViewports(1, &viewport);
    memcpy(clear, smile_frame_clear3d, sizeof(clear));
    if (smile_hdr_effective3d)
        for (int component = 0; component < 3; ++component)
            clear[component] = clear[component] <= 0.04045f
                ? clear[component] / 12.92f
    ...
    if (success)
        for (unsigned int index = 0; index < smile_frame_submission_count3d; ++index)
        {
            const SmileSubmission3D* submission = &smile_frame_submissions3d[index];
            if (submission->kind != SMILE_3D_SUBMISSION_OBJECT ||
                submission->object.reflection_mode != 1 ||
                !smile_3d_submission_is_opaque(submission))
                continue;
    ...
    if (!success)
        smile_reflections_skip(SMILE_3D_REFLECTION_FALLBACK_RENDER_FAILED);
    else
        smile_reflections_complete_capture(0, 0);
    return 1;
}
...
static int smile_3d_submission_is_opaque(const SmileSubmission3D* submission)
{
    return submission->kind == SMILE_3D_SUBMISSION_OBJECT &&
        submission->alpha_mode <= 1;
}

static int smile_3d_submission_is_distortion(const SmileSubmission3D* submission)
{
    return submission->kind != SMILE_3D_SUBMISSION_OBJECT &&
        submission->has_material &&
        submission->material.vfx_shading_mode == SMILE_3D_VFX_SHADING_DISTORTION;
}
...
static int smile_3d_snapshot_linear_depth(void)
{
    ID3D11DeviceContext* context = (ID3D11DeviceContext*)smile_graphics_directx_context();
    ID3D11ShaderResourceView* no_view = 0;
    D3D11_VIEWPORT viewport = {};
    SmileDepthConstants3D constants = {};
    float clear[4] = { smile_camera_far3d, 0.0f, 0.0f, 0.0f };
    if (smile_soft_depth_effective3d == SMILE_3D_SOFT_DEPTH_OFF) return 1;
    if (context == 0 || smile_depth_shader_view3d == 0 ||
        smile_linear_depth_view3d == 0 || smile_linear_depth_shader_view3d == 0)
    {
        smile_soft_depth_copy_failure_count3d++;
        smile_soft_depth_effective3d = SMILE_3D_SOFT_DEPTH_OFF;
        smile_soft_depth_fallback_reason3d = SMILE_3D_SOFT_DEPTH_FALLBACK_TARGET;
    ...
    context->OMSetRenderTargets(0, 0, 0);
    context->PSSetShaderResources(0, 1, &smile_depth_shader_view3d);
    context->OMSetRenderTargets(1, &smile_linear_depth_view3d, 0);
    context->ClearRenderTargetView(smile_linear_depth_view3d, clear);
    viewport.Width = (FLOAT)smile_soft_depth_width3d;
    viewport.Height = (FLOAT)smile_soft_depth_height3d;
    viewport.MinDepth = 0.0f;
    viewport.MaxDepth = 1.0f;
    context->RSSetViewports(1, &viewport);
    context->OMSetDepthStencilState(0, 0);
    ...
    context->IASetInputLayout(0);
    context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    context->VSSetShader(smile_depth_copy_vertex_shader3d, 0, 0);
    context->PSSetShader(smile_sample_count3d > 1
        ? smile_depth_copy_msaa_pixel_shader3d : smile_depth_copy_pixel_shader3d, 0, 0);
    ...
    context->PSSetShaderResources(0, 1, &no_view);
    context->OMSetRenderTargets(0, 0, 0);
    smile_soft_depth_copy_draw_count3d++;
    return 1;
}
...
static int smile_3d_render_distortion_pass(void)
{
    ID3D11DeviceContext* context =
        (ID3D11DeviceContext*)smile_graphics_directx_context();
    ID3D11ShaderResourceView* no_views[7] = {};
    D3D11_VIEWPORT viewport = {};
    SmilePostConstants3D constants = {};
    float clear[4] = {};
    unsigned int submission;
    if (smile_distortion_effective3d == SMILE_3D_DISTORTION_OFF) return 1;
    if (context == 0 || smile_distortion_view3d == 0 ||
        smile_distortion_shader_view3d == 0 || smile_distortion_scratch_view3d == 0 ||
        smile_scene_shader_view3d == 0)
    {
        smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
        smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_TARGET;
        return 1;
    ...
    viewport.MaxDepth = 1.0f;
    context->RSSetViewports(1, &viewport);
    smile_rendering_distortion_vectors3d = 1;
    for (submission = 0; submission < smile_frame_submission_count3d; ++submission)
    {
        const SmileSubmission3D* entry = &smile_frame_submissions3d[submission];
        if (!smile_3d_submission_is_distortion(entry)) continue;
        if (!smile_3d_draw_submission(entry))
        {
            smile_rendering_distortion_vectors3d = 0;
            return 0;
        }
        smile_distortion_emitter_count3d++;
        smile_distortion_vector_draw_count3d++;
        if ((int)entry->material.distortion_strength >
            smile_distortion_maximum_strength3d)
            smile_distortion_maximum_strength3d =
                (int)entry->material.distortion_strength;
    }
    for (submission = 0; submission < smile_gpu_particle_frame_count3d; ++submission)
    {
        SmileGpuParticleSystem3D* system = smile_3d_gpu_particle_system(
        ...
        smile_distortion_emitter_count3d++;
        smile_distortion_vector_draw_count3d++;
        if ((int)material->distortion_strength > smile_distortion_maximum_strength3d)
            smile_distortion_maximum_strength3d =
                (int)material->distortion_strength;
    }
    smile_rendering_distortion_vectors3d = 0;
    context->PSSetShaderResources(0, 7, no_views);
    context->OMSetRenderTargets(0, 0, 0);
    ...
    smile_3d_bind_post_pass(context, smile_distortion_scratch_view3d,
        ...
        smile_graphics_directx_physical_height(), smile_scene_shader_view3d,
        smile_distortion_shader_view3d, &constants);
    context->PSSetShaderResources(0, 7, no_views);
    context->OMSetRenderTargets(0, 0, 0);
    ...
    }
    {
        ...
        ID3D11ShaderResourceView* view = smile_scene_shader_view3d;
        smile_color_texture3d = smile_distortion_scratch_texture3d;
        smile_color_view3d = smile_distortion_scratch_view3d;
        smile_scene_shader_view3d = smile_distortion_scratch_shader_view3d;
        smile_distortion_scratch_texture3d = texture;
        smile_distortion_scratch_view3d = target;
        smile_distortion_scratch_shader_view3d = view;
    }
    return 1;
}
...
static int smile_3d_end(void)
{
    ...
    if (success && smile_multipass_active3d && context != 0)
    {
        ...
        if (success) success = smile_3d_render_reflection_pass();
        target = smile_color_view3d != 0
            ? smile_color_view3d
            : (ID3D11RenderTargetView*)smile_graphics_directx_render_target();
        context->OMSetRenderTargets(1, &target, smile_depth_view3d);
        ...
        if (success)
            for (unsigned int submission = 0;
                ...
                if (smile_3d_submission_is_opaque(&smile_frame_submissions3d[submission]) &&
        ...
        if (success)
            for (unsigned int submission = 0;
                submission < smile_frame_submission_count3d; ++submission)
                if (!smile_3d_submission_is_opaque(&smile_frame_submissions3d[submission]) &&
                    !smile_3d_submission_is_distortion(&smile_frame_submissions3d[submission]) &&
                    !smile_3d_draw_submission(&smile_frame_submissions3d[submission]))
                { success = 0; break; }
        if (success)
            for (unsigned int submission = 0;
                submission < smile_gpu_particle_frame_count3d; ++submission)
            {
                SmileGpuParticleSystem3D* system = smile_3d_gpu_particle_system(
                    smile_gpu_particle_frame_handles3d[submission]);
                if (system != 0 && !smile_3d_gpu_particle_is_distortion(system) &&
                    !smile_3d_draw_gpu_particle_system(system))
                { success = 0; break; }
            }
    }
    else if (success && context != 0)
    {
        for (unsigned int submission = 0;
            submission < smile_gpu_particle_frame_count3d; ++submission)
        {
            SmileGpuParticleSystem3D* system = smile_3d_gpu_particle_system(
                smile_gpu_particle_frame_handles3d[submission]);
            if (system != 0 && !smile_3d_draw_gpu_particle_system(system))
            { success = 0; break; }
        }
    }
    if (context != 0)
    {
        ID3D11ShaderResourceView* empty_views[7] = {};
        context->PSSetShaderResources(0, 7, empty_views);
        context->OMSetRenderTargets(0, 0, 0);
        if (success && smile_hdr_effective3d)
            success = smile_3d_run_post_processing();
        else if (smile_color_texture3d != 0)
        {
            ...
            if (destination != 0)
            {
                ...
                else context->CopyResource(destination, smile_color_texture3d);
                destination->Release();
            }
        }
        ID3D11Buffer* empty_buffers[2] = {};
        ID3D11SamplerState* empty_samplers[7] = {};
        ...
        context->PSSetSamplers(0, 7, empty_samplers);
        context->OMSetBlendState(0, 0, 0xffffffff);
        context->OMSetDepthStencilState(0, 0);
...
extern "C" void smile_graphics3d_on_device_lost(void)
{
    ...
    smile_3d_release(smile_gpu_particle_input_layout3d);
    smile_3d_release(smile_gpu_particle_vertex_shader3d);
    smile_3d_release(smile_gpu_particle_compute_shader3d);
    ...
    smile_3d_release(smile_depth_copy_pixel_shader3d);
    smile_3d_release(smile_depth_copy_vertex_shader3d);
    targets = smile_3d_take_target_state();
    smile_3d_clear_target_state();
    smile_3d_release_target_state(targets);
    smile_3d_release(smile_linear_depth_shader_view3d);
    smile_3d_release(smile_linear_depth_view3d);
    smile_3d_release(smile_linear_depth_texture3d);
    smile_3d_release(smile_depth_shader_view3d);
    ...
    for (index = 0; index < SMILE_3D_MAX_GPU_PARTICLE_SYSTEMS; ++index)
    {
        SmileGpuParticleSystem3D* system = &smile_gpu_particle_systems3d[index];
        if (!system->active) continue;
        if (system->effective_simulation == SMILE_3D_GPU_PARTICLE_SIMULATION_FAST)
        {
            ZeroMemory(system->states[0],
                sizeof(SmileGpuParticleState3D) * system->capacity);
            ZeroMemory(system->states[1],
                sizeof(SmileGpuParticleState3D) * system->capacity);
            ZeroMemory(system->slot_ages, sizeof(unsigned int) * system->capacity);
            ZeroMemory(system->slot_lifetimes, sizeof(unsigned int) * system->capacity);
            ZeroMemory(system->slot_active, sizeof(unsigned char) * system->capacity);
            system->active_count = 0;
            system->pending_count = 0;
            system->accumulator_ms = 0;
            system->read_index = 0;
            system->read_generation++;
            if (system->read_generation == 0) system->read_generation = 1;
            system->write_generation = system->read_generation + 1;
            if (system->write_generation == 0) system->write_generation = 1;
            smile_gpu_particle_restart_count3d++;
        }
        smile_3d_release_gpu_particle_resources(system);
    }
    smile_gpu_particle_pipeline_state3d =
        SMILE_3D_GPU_PARTICLE_PIPELINE_NOT_ATTEMPTED;
    smile_target_width3d = smile_target_height3d = 0;
    ...
    smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
    smile_distortion_fallback_reason3d = smile_distortion_requested3d
        ? SMILE_3D_DISTORTION_FALLBACK_NONE
        : SMILE_3D_DISTORTION_FALLBACK_DISABLED;
    smile_distortion_width3d = smile_distortion_height3d = 0;
    smile_distortion_bytes3d = 0;
    smile_rendering_distortion_vectors3d = 0;
    ...
    long long handle, long long slot)
{
    ...
    if (query == 3) return SMILE_3D_MAX_GPU_REFERENCE_PARTICLES;
    if (query == 60) return SMILE_3D_MAX_GPU_PARTICLE_CAPACITY;
    if (query == 4) return SMILE_3D_GPU_PARTICLE_STATE_SCHEMA;
    if (query == 5) return sizeof(SmileGpuParticleState3D);
    if (query == 6) return smile_gpu_particle_pipeline_state3d ==
        SMILE_3D_GPU_PARTICLE_PIPELINE_AVAILABLE
        ? SMILE_3D_GPU_PARTICLE_BACKEND_D3D11 : SMILE_3D_GPU_PARTICLE_BACKEND_OFF;
    if (query == 7) return smile_gpu_particle_pipeline_state3d ==
        SMILE_3D_GPU_PARTICLE_PIPELINE_AVAILABLE
        ? SMILE_3D_GPU_PARTICLE_FALLBACK_NONE :
            SMILE_3D_GPU_PARTICLE_FALLBACK_BACKEND_UNAVAILABLE;
    ...
    if (query == 14) return smile_gpu_particle_dispatch_count3d;
    if (query == 15) return smile_gpu_particle_render_draw_count3d;
    if (query == 16) return smile_gpu_particle_cpu_upload_bytes3d;
    if (query == 17) return smile_gpu_particle_gpu_state_bytes3d;
    if (query == 18) return smile_gpu_particle_restart_count3d;
    ...
    if (query >= 30 && query <= 50)
    {
        ...
        if (query == 46) return
            (long long)system->capacity *
                (sizeof(SmileGpuParticleState3D) * 3 + sizeof(unsigned int) * 3 +
                    sizeof(unsigned char)) +
            sizeof(SmileGpuParticleSpawnCommand3D) * SMILE_3D_MAX_GPU_SPAWN_COMMANDS;
        if (query == 47) return system->gpu_states[0] == 0 ? 0 :
            (long long)system->capacity * sizeof(SmileGpuParticleState3D) * 2 +
                sizeof(SmileGpuParticleConstants3D);
        ...
        }
        {
            ...
            if (query == 42) return system->slot_ages[slot];
            if (query == 43) return system->slot_lifetimes[slot];
...
static void smile_3d_reset(void)
{
    ...
    smile_gpu_particle_dispatch_count3d = 0;
    smile_gpu_particle_render_draw_count3d = 0;
    smile_gpu_particle_gpu_state_bytes3d = 0;
    smile_gpu_particle_restart_count3d = 0;
    ...
    smile_soft_depth_bytes3d = 0;
    smile_soft_depth_copy_draw_count3d = 0;
    smile_soft_depth_copy_failure_count3d = 0;
    smile_soft_particle_draw_count3d = 0;
    smile_distortion_requested3d = 0;
    smile_distortion_quality3d = 3;
    smile_distortion_effective3d = SMILE_3D_DISTORTION_OFF;
    smile_distortion_fallback_reason3d = SMILE_3D_DISTORTION_FALLBACK_DISABLED;
    smile_distortion_width3d = smile_distortion_height3d = 0;
    smile_distortion_bytes3d = 0;

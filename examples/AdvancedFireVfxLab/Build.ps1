[CmdletBinding()]
param([string]$OutputPath = (Join-Path $PSScriptRoot 'bin\Debug\AdvancedFireVfxLab.exe'))
...
& (Join-Path $taskRoot 'artifacts\compiler\smilec.exe') --project (Join-Path $PSScriptRoot 'AdvancedFireVfxLab.smileproj') `
    --target windows-x64 --configuration Debug --graphics DirectX -o $OutputPath
if ($LASTEXITCODE -ne 0) { throw 'Advanced Fire Lab compilation failed.' }

...
Phase 5.2.2 project/package acceptance client for the reusable `Smile.UI.MenuNavigator` library.
...
The sample stores plain labels. `MenuNavigator` bindings let `Smile.UI.Menu` hide or draw the exact literal ` >` after fitted text or in a reserved right-side region. Every visible menu draws its own vertically centered cursor while only the deepest menu receives keyboard input; every row uses the same fixed cursor gutter so text never shifts.

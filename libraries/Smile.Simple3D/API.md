...
Shared screen-fixed image backgrounds for Character Viewer and Fire Lab; no camera-facing plane, world transform, per-frame image upload, or orbit-dependent UV calculations. Native and WebGL2 draw the backdrop behind the scene, then the existing post-processing and Renderer2D HUD run normally. Local heat refraction can affect pixels near a fire; it does not move the background with the camera. Web uses the existing post pipeline with one sRGB decode for HDR, and preserves selection across frames. Invalid handles still fail instead of being relabeled as successful clear-color rendering.

...
- The native and Web renderers reuse immutable accepted frame submissions for one mirrored-camera pass. Opaque and alpha-masked simple/PBR objects participate; alpha-blended effects do not. When backdrop inclusion is requested, its flat screen-fixed image is mapped from only the source region visible above the topmost projected receiver edge, while the normal scene backdrop remains unchanged. Floor-covered backdrop pixels are never exposed as new scene content. No actor, animation, event, audio, or VFX update is repeated.
...
- `SetPartVisible` provides bounded per-part visibility for editor and equipment workflows. Its intent survives placement, rotation, scaling and whole-actor hide/show; a hidden actor always hides every part. A failed visibility write does not commit new intent. `PrimaryObjectHandle`, indexed `PartObjectHandle`, `AnimatorHandle`, and `ModelHandle` are borrowed read-only advanced Battle3D interop values. Callers must not destroy them or mutate Character3D-owned transforms. Character3D does not depend on Battle3D.
...
Query requested/effective backend and quality separately with `Value`. CPU Low intentionally lacks GPU thermal/turbulence and heat-distortion parity; capable native and WebGL2 paths use genuine High/Medium GPU layers. GPU/Auto systems admit 16,384 slots; explicitly requested CPU-reference systems remain capped at 8,192, with the same 32-system shared limit. `GPU_PARTICLE_QUERY_MAX_TOTAL_CAPACITY` (60) exposes the existing 32,768-slot ceiling without changing any command ID. Bounds follow current/recent segment positions with radius/lifetime/velocity padding, rather than covering the entire world. Zero elapsed time does not advance or consume source history.

Native and WebGL2 thermal dynamics expose wind, gravity, buoyancy, dt-aware drag, seeded 1/2-octave turbulence, cooling, dissipation, size evolution, speed limits and kill bounds through the Graphics3D GPU-particle setters. Particle simulation has no GPU state readback. Both targets retain supported 4x/2x/1x scene MSAA in HDR and LDR: resolve the opaque snapshot, distort it, copy color back without tone mapping, preserve MSAA depth for transparent draws, then perform normal final resolve/post-processing. WebGL2 selects the intersection of color/depth sample support, validates each target, and falls back to fewer samples on allocation failure. `M5_QUERY_SAMPLE_COUNT`, `M5_QUERY_RESOLVE_COUNT`, scene bytes and fallback flags report the actual path. The Web LDR presentation uses one full-screen texture-copy draw; it does not tone-map the scene. Multisample targets are reused until a configuration or backing-size change and retired with the existing target bundle on reset or replacement.

## `Smile.Simple3D.StaticBackdrop3D`

Shared screen-fixed image backgrounds for Character Viewer and Fire Lab; no camera-facing plane, world transform, per-frame image upload, or orbit-dependent UV calculations. Native and WebGL2 draw the backdrop behind the scene, then the existing post-processing and Renderer2D HUD run normally. A planar-reflection pass can map the visible above-receiver region of that same flat image into its receiver without changing the main backdrop or revealing pixels covered by the floor. Local heat refraction can affect pixels near a fire; it does not move the background with the camera. Web uses the existing post pipeline with one sRGB decode for HDR, and preserves selection across frames. Invalid handles still fail instead of being relabeled as successful clear-color rendering.

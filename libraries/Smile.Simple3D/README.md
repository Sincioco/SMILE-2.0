...
Character3D transform changes are transactional across every model part. World position is bounded to +/-1,000,000, rotation input is bounded and normalized to 0-359 degrees, and scale is 1-10,000 percent. The extended scale range lets meter-scale imported characters use a larger integer-world scene for smooth camera motion. Advanced part/model/animator handles are borrowed read-only values; destroying them deliberately is treated as external tampering and quarantines only the affected actor or asset.

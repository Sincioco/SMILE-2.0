...
Successful publication writes `<executable-base>.smile-assets.json` for native output or `smile-assets.json` for Web output. A later build uses only that validated manifest to remove stale managed assets; generated compiler output and unrelated files remain untouched. Publishing multiple applications into one identical output directory is not recommended because application-level destination conflicts are outside this milestone's ownership model.

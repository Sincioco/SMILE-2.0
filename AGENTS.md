...
- Before future Arin model/import/export/rig/attachment/calibration/VFX work, read `games\SinStarI\SourceAssets\Characters\Paladin\ArinV57\ARIN-CREATION-AND-REPAIR-JOURNEY.md`. Keep its learned workflow and version-specific caveats current; do not treat historical offsets or superseded controls as new defaults.
...
- Keep Orin v1.3 self-contained in `games\SinStarI\SourceAssets\Characters\Tank\OrinV13`. Character Viewer tabs and Party playback must use each character's own assets, animation clips, equipment, and calibration storage. The shared calibration synchronizer accepts `-Character Orin`; export both characters before commits containing their calibration changes.
...
- Precise camera/transform code follows the [Precision3D boundary](docs/libraries/precision3d-boundary.md); preserve accepted asset scale instead of rebaking or rescaling assets to hide quantization.
